...
extern struct sbi_ecall_extension ecall_cpu;
static struct sbi_ecall_extension *sbi_ecall_exts[] = {&ecall_base, &ecall_cpu,
                                                       NULL};

u16 sbi_ecall_version_major(void) { return SBI_ECALL_VERSION_MAJOR; }

u16 sbi_ecall_version_minor(void) { return SBI_ECALL_VERSION_MINOR; }

static unsigned long ecall_impid = SBI_OPENSBI_IMPID;

unsigned long sbi_ecall_get_impid(void) { return ecall_impid; }

void sbi_ecall_set_impid(unsigned long impid) { ecall_impid = impid; }
...
struct sbi_ecall_extension *sbi_ecall_find_extension(unsigned long extid) {
  ...
  list_for_each_entry(t, &ecall_exts_list, head) {
...
int sbi_ecall_handler(struct pt_regs *regs) {
  ...
  if (ret == SBI_ETRAP) {
    ...
    if (ret < SBI_LAST_ERR) {
      printf("%s: Invalid error %d for ext=0x%lx "
             "func=0x%lx\n",
