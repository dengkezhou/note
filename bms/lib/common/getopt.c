  ...
  if (longopts != NULL &&
    ...
    if (ambig_list != NULL && !exact) {
      if (print_errors) {
        ...
        if (fp != NULL) {
          printf(fp, _("%s: option '%s' is ambiguous; possibilities:"), argv[0],
                 argv[d->optind]);

          do {
            printf(fp, " '--%s'", ambig_list->p->name);
...
#else
        printf("%s: option '%s' is ambiguous; possibilities:", argv[0],
               argv[d->optind]);
        do {
          printf(" '--%s'", ambig_list->p->name);
          ambig_list = ambig_list->next;
        } while (ambig_list != NULL);
        printf("\n");
#endif
