...
void CYGACC_COMM_IF_PUTC(char x, char y) { putc(y); }
...
static void zm_new(void) { zm_bp = zm_buf; }

static void zm_save(unsigned char c) { *zm_bp++ = c; }
...
void xyzModem_stream_terminate(bool abort, int (*getc)(void)) {
  ...
  if (abort) {
    ...
    case xyzModem_ymodem:
      /* The X/YMODEM Spec seems to suggest that multiple CAN followed by an
       * equal */
...
unsigned char Eot_Send            = 0;
unsigned char FirstPackSend       = 0;
unsigned char SOH_Idx             = 0;
unsigned int blk_num_send         = 0;
unsigned char file_send_end       = 0;
unsigned char file_send_end_ACK   = 0;
unsigned char file_name_send_ACK  = 0;
unsigned char FirstSend           = 1;
unsigned char PacketStartFlagSend = 0;
unsigned char No_More_File        = 0;
unsigned char Empty_Ind           = 0;
unsigned int NumBlks;
unsigned int LenLastBlk;
unsigned char SOHLastBlk;

int real_send(char *mem, int count);
int procsend(char *buf, char *mem, int count);
int Procpacket(char *buf, char *mem, int count);
...
void out_message(void) {
  printf("Eot_Send= %d\n", Eot_Send);
  printf("FirstPackSend= %d\n", FirstPackSend);
  printf("blk_num_send= %d\n", blk_num_send);
  printf("file_send_end= %d\n", file_send_end);
  printf("file_send_end_ACK= %d\n", file_send_end_ACK);
  printf("file_name_send_ACK= %d\n", file_name_send_ACK);
  printf("FirstSend= %d\n", FirstSend);
  printf("PacketStartFlagSend= %d\n", PacketStartFlagSend);
  printf("No_More_File= %d\n", No_More_File);
  printf("Empty_Ind= %d\n", Empty_Ind);
}
...
int control_send(char *mem, int count) {
  ...
  for (;;) {
    ...
    for (; send_next;) {
      ...
      if (!Empty_Ind) {
        if (file_send_end) {
          CYGACC_COMM_IF_PUTC(0, EOT);
          Eot_Send = 1;
          continue;
        }
      }
      send_idx = real_send(mem, count);
      if (send_idx == ALL_NEED_OFFSET)
        continue;
      else if (send_idx != 0)
        return send_idx;
      if (PacketStartFlagSend)
...
int real_send(char *mem, int count) {
  ...
  if (Empty_Ind)
    ...
    blk_num_send++;
  frame_idx = procsend(send_buf, mem, count);
  if (frame_idx)
    return frame_idx;
  if (SOH_Idx == 1)
    packet_len = 132;
  ...
  for (i = 0; i < packet_len; i++)
    CYGACC_COMM_IF_PUTC(0, send_buf[i]);
  if (No_More_File) {
    delay(10000000);
    return -1;
  }
  if (blk_num_send == NumBlks) {
    file_send_end = 1;
    return ALL_NEED_OFFSET;
  }

  return 0;
}

int procsend(char *send_buf, char *mem, int count) {
  ...
  if (FirstSend) {
    FileLenBkup = FileLen = count;
    LenLastBlk            = FileLen % 1024;
    NumBlks               = (FileLen >> 10);

    filename_real = "yl.dump";
    if (LenLastBlk) {
      NumBlks += 1;
      if (LenLastBlk < 128)
        SOHLastBlk = 1;
    ...
    if (blk_num_send <= NumBlks) {
      if (Empty_Ind)
        ...
        SOH_Idx = 2;
      packet_ind = Procpacket(send_buf, mem, count);
    }
    return (packet_ind);
  }
}

int Procpacket(char *send_buf, char *mem, int count) {
  ...
  if (!file_send_end)
    memcpy(send_buf + 3, mem + FileLenBkup - FileLen, size_read);
  FileLen -= size_read;
  if (Empty_Ind) {
