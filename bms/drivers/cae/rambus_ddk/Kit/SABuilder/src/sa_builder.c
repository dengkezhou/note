...
SABuilder_SetCipherKeys(void *ctx,
        ...
        uint32_t * const SABuffer_p)
{
	...
	if (((cipher_ctx *)ctx)->pad == PADDING_PKCS7)
		SAState_p->CW1 |= SAB_CW1_PAD_PKCS7;
	else if (((cipher_ctx *)ctx)->pad == PADDING_ZERO)
		SAState_p->CW1 |= SAB_CW1_PAD_ZERO;

	SAState_p->CW1 |= SAB_CW1_IV0 | SAB_CW1_IV1;//| SAB_CW1_IV2 | SAB_CW1_IV3;
    }
        ...
        unsigned int *const ARC4StateWord32Count_p)
{
    ...
    rc = SABuilder_SetCipherKeys(ctx, SAParams_p, &SAState, NULL);
    if (rc != SAB_STATUS_OK)
        return rc;
        ...
        uint32_t *const ARC4StateBuffer_p)
{
    ...
    rc = SABuilder_SetCipherKeys(ctx, SAParams_p, &SAState, SABuffer_p);
    if (rc != SAB_STATUS_OK)
        return rc;
...
#ifdef SAB_ENABLE_FIXED_RECORD_SIZE
#ifdef SAB_ENABLE_TWO_FIXED_RECORD_SIZES
            if (SAState.CurrentOffset + 2> SAB_LARGE_RECORD_THRESHOLD_WORD_COUNT)
            {
                ARC4Offset = SAB_RECORD_WORD_COUNT_LARGE;
            }
            else
#else
            {
	...
	printf("SABuffer_p[0] addr: 0x%x, SABuffer_p[0]: 0x%x\n", &SABuffer_p[0], SABuffer_p[0]);
#endif
	if (SAParams_p->CryptoAlgo == SAB_CRYPTO_DES ||
        SAParams_p->CryptoAlgo == SAB_CRYPTO_3DES ||
        SAParams_p->CryptoAlgo == SAB_CRYPTO_AES) {
		if (((cipher_ctx *)ctx)->pad == PADDING_PKCS7)
			SAState.CW1 |= SAB_CW1_PAD_PKCS7;
		else if (((cipher_ctx *)ctx)->pad == PADDING_ZERO)
			SAState.CW1 |= SAB_CW1_PAD_ZERO;

		SAState.CW1 |= SAB_CW1_IV0 | SAB_CW1_IV1;//| SAB_CW1_IV2 | SAB_CW1_IV3;
	}
