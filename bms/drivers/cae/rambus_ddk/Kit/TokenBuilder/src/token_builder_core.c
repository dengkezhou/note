    ...
    uint32_t * const TokenHeaderWord_p)
{
	...
	if (((cipher_ctx *)ctx)->final_pkt == 1) {
		if (((cipher_ctx *)ctx)->pad == PADDING_PKCS7) {
			if (EVAL_packetsize() == TokenContext_Internal_p->PadBlockByteCount) {
				if (((cipher_ctx *)ctx)->enc == SAB_DIRECTION_OUTBOUND)
					pad_bytes_basic = TokenContext_Internal_p->PadBlockByteCount;
				else
					pad_bytes_basic = 0;
			} else {
	...
	if (((cipher_ctx *)ctx)->pad == PADDING_PKCS7)
		*tp++=0x2d0e0000 | ((0u)&0x1u)<<16 | ((pad_bytes_basic)&0xffffu)<<0;
