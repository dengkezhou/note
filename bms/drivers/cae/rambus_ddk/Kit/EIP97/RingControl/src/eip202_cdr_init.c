 ...
 */
static bool
EIP202Lib_CDR_Detect(cae_device *dev,
        const Device_Handle_t Device)
{
    ...
    Value = EIP202_CDR_Read32(dev, Device, EIP202_CDR_RING_BASE_ADDR_LO);
    if ((Value & MASK_31_BITS) != MASK_31_BITS)
        return false;

    // Clear MASK_31_BITS bits of the EIP202_CDR_RING_BASE_ADDR_LO register
    EIP202_CDR_Write32(dev, Device, EIP202_CDR_RING_BASE_ADDR_LO, 0);
    Value = EIP202_CDR_Read32(dev, Device, EIP202_CDR_RING_BASE_ADDR_LO);
 ...
 */
static inline void
EIP202Lib_CDR_ClearAllDescriptors(cae_device *dev,
 ...
 */
EIP202_Ring_Error_t
EIP202_CDR_Init(cae_device *dev,
        ...
        const EIP202_CDR_Settings_t * const CDRSettings_p)
{
    ...
    if(!EIP202Lib_CDR_Detect(dev, Device))
        return EIP202_RING_UNSUPPORTED_FEATURE_ERROR;
    ...
    EIP202_CDR_STAT_FIFO_SIZE_RD(dev, Device, &CDFIFOWordCount);
    ...
    EIP202Lib_CDR_ClearAllDescriptors(dev,
    ...
    EIP202_CDR_RING_BASE_ADDR_LO_WR(dev,
                       Device,
                       CDRSettings_p->Params.RingDMA_Address.Addr);

    EIP202_CDR_RING_BASE_ADDR_HI_WR(dev,
                       Device,
                       CDRSettings_p->Params.RingDMA_Address.UpperAddr);

    EIP202_CDR_RING_SIZE_WR(dev,
                       Device,
                       CDRSettings_p->Params.RingSizeWordCount);

    EIP202_CDR_DESC_SIZE_WR(dev,
                       Device,
                       CDRSettings_p->Params.DscrSizeWordCount,
                       CDRSettings_p->Params.DscrOffsWordCount,
 ...
 */
EIP202_Ring_Error_t
EIP202_CDR_Reset(cae_device *dev,
        ...
        const Device_Handle_t Device)
{
    ...
    // Clear CDR count
    EIP202_CDR_COUNT_WR(dev, Device, 0, true);

    // Re-init CDR
    EIP202_CDR_POINTER_DEFAULT_WR(dev, Device);

    // Restore default register values
    EIP202_CDR_RING_BASE_ADDR_LO_DEFAULT_WR(dev, Device);
    EIP202_CDR_RING_BASE_ADDR_HI_DEFAULT_WR(dev, Device);
    EIP202_CDR_RING_SIZE_DEFAULT_WR(dev, Device);
    EIP202_CDR_DESC_SIZE_DEFAULT_WR(dev, Device);
    EIP202_CDR_CFG_DEFAULT_WR(dev, Device);
    EIP202_CDR_DMA_CFG_DEFAULT_WR(dev, Device);
 ...
 */
EIP202_Ring_Error_t
EIP202_CDR_Options_Get(cae_device *dev,
        const Device_Handle_t Device,
        EIP202_Ring_Options_t * const Options_p)
{
    ...
    Rev = EIP202_CDR_Read32(dev, Device, EIP202_CDR_VERSION);
    ...
    EIP202_CDR_OPTIONS_RD(dev, Device,
                          &Options_p->NofRings,
                          &Options_p->NofPes,
                          &Options_p->fExpPlf,
                          &Options_p->CF_Size,
