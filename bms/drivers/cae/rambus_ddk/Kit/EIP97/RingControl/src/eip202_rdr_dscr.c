...
EIP202Lib_RDR_Prepared_FillLevel_Get(cae_device *dev,
        ...
        unsigned int * const FillLevelDscrCount_p)
{
    ...
    EIP202_RDR_PREP_COUNT_RD(dev, Device, (uint32_t*)&RDWordCount);
        ...
        const unsigned int DescriptorSkipCount)
{
    ...
    if (nWritten > 0)
    {
        ...
        EIP202_RDR_PREP_COUNT_WR(dev, Device,
...
#define EIP202_RING_RDR_ALL_PACKETS_TO_GET_DONE                 \
        ...
        DMAResource_PostDMA(dev, TrueIOArea_p->RingHandle,
                            i * DescOffsetWordCount *
                              (unsigned int)sizeof(uint32_t),
                            DescOffsetWordCount *
                              (unsigned int)sizeof(uint32_t));
...
#if defined(EIP202_RDR_OWNERSHIP_WORD_ENABLE)
        {
            uint32_t OwnershipWord =
                        DMAResource_Read32(dev, TrueIOArea_p->RingHandle,
                                           i * DescOffsetWordCount +
...
#else
        fGotDescriptor = true; // according to EIP202_RDR_PROC_COUNT_RD(dev, )
#endif

        if (fGotDescriptor)
        {
            // Read Result Descriptor
            EIP202_ReadDescriptor(dev, CurrentResultDesc_p,
                                  TrueIOArea_p->RingHandle,
                                  i * DescOffsetWordCount,
                                  DescOffsetWordCount,
                                  &fLastSegment,
    ...
    defined(EIP202_RDR_OWNERSHIP_WORD_ENABLE)
            ...
            EIP202_ClearDescriptor(dev, CurrentResultDesc_p,
                                   TrueIOArea_p->RingHandle,
                                   i * DescOffsetWordCount,
                                   DescOffsetWordCount);

            // Ensure next PostDMA does not undo the clear operation above
            DMAResource_PreDMA(dev,
                TrueIOArea_p->RingHandle,
 ...
 */
EIP202_Ring_Error_t
EIP202_RDR_Prepared_FillLevel_Get(cae_device *dev,
        EIP202_Ring_IOArea_t * const IOArea_p,
        unsigned int * FillLevelDscrCount_p)
{
    Device_Handle_t Device;
    ...
    return EIP202Lib_RDR_Prepared_FillLevel_Get(dev, Device,
                                               TrueIOArea_p,
                                               FillLevelDscrCount_p);
}
 ...
 */
EIP202_Ring_Error_t
EIP202_RDR_Descriptor_Prepare(cae_device *dev,
        EIP202_Ring_IOArea_t * const IOArea_p,
...
// If configured then the driver cannot rely on the register counter
...

    {
        ...
        EIP202_RDR_PROC_COUNT_RD(dev, Device, &Value32, &Value8);
 ...
 */
EIP202_Ring_Error_t
EIP202_RDR_Descriptor_Get(cae_device *dev,
        EIP202_Ring_IOArea_t * const IOArea_p,
        ...
        unsigned int * FillLevelDscrCount_p)
{
    ...
    res = RingHelper_Get(dev, (volatile RingHelper_t*)&TrueIOArea_p->RingHelper,
// If configured then the driver cannot rely on the register counter
#if defined(EIP202_RING_ANTI_DMA_RACE_CONDITION_CDS) || \
    defined(EIP202_RDR_OWNERSHIP_WORD_ENABLE)
