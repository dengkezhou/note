 ...
 */
static int
GlobalControl97Lib_Resume(cae_device *dev, void * p)
{
    unsigned int i;
    ...
    rc = EIP97_Global_Init(dev, &dev->Global_IOArea,
                           Device_Find(dev, ADAPTER_GLOBAL_DEVICE_NAME));
    ...
    for (i = 0; i < ADAPTER_GLOBAL_EIP97_NOF_PES; i++)
    {
        if (GlobalControl97_RingPEMap[i].fCached)
        {
            ...
            rc = EIP97_Global_Configure(dev, &dev->Global_IOArea,
                                        i,
        ...
        if (GlobalControl97_Prng[i].fCached && dev->Global_PRNG_Present)
        {
            LOG_INFO("\n\t\t\t\t EIP97_Global_PRNG_Reseed \n");

            rc = EIP97_Global_PRNG_Reseed(dev, &dev->Global_IOArea,
                                          i,
        ...
        GlobalControl97_Capabilities_t * const Capabilities_p)
{
...

    {
        ...
        rc = EIP97_Global_HWRevision_Get(dev, &dev->Global_IOArea, &Capabilities);
 ...
 */
GlobalControl97_Error_t
GlobalControl97_Init(cae_device *c_dev,
        const bool fHWResetDone)
{
    ...
    if (!fHWResetDone)
    {
        ...
        rc = EIP97_Global_Reset(c_dev, &c_dev->Global_IOArea, dev);
        if (rc == EIP97_GLOBAL_BUSY_RETRY_LATER)
        {
            ...
            do {
                ...
                rc = EIP97_Global_Reset_IsDone(c_dev, &c_dev->Global_IOArea);
                if (rc != EIP97_GLOBAL_NO_ERROR &&
    ...
    rc = EIP97_Global_HWRevision_Get(c_dev, &c_dev->Global_IOArea, &Capabilities);
    if (rc != EIP97_GLOBAL_NO_ERROR)
    {
        LOG_CRIT("GlobalControl97_Init: returned error");
        goto exit; // error
    }
    c_dev->Global_PRNG_Present = !Capabilities.EIP97_Options.central_prng;

    if (GlobalControl97Lib_Resume(c_dev, NULL) != 0)
        goto exit; // error
    else
    {
        c_dev->Global_IsInitialized = true;
        ...
        Adapter_Ring_EIP202_Configure(c_dev, Capabilities.EIP202_Options.HDW,
                                      Capabilities.EIP202_Options.CF_Size,
                                      Capabilities.EIP202_Options.RF_Size);
#endif
...
GlobalControl97_UnInit(cae_device *c_dev)
{
    ...
    c_dev->Global_IsInitialized = false;
        ...
        const GlobalControl97_Ring_PE_Map_t * const RingPEMap_p)
{
    ...
    rc = EIP97_Global_Configure(dev, &dev->Global_IOArea, PE_Number, RingPEMap_p);
        ...
        const GlobalControl97_PRNG_Reseed_t * const ReseedData_p)
{
    ...
    rc = EIP97_Global_PRNG_Reseed(dev, &dev->Global_IOArea, PE_Number, ReseedData_p);
        ...
        GlobalControl97_DFE_Status_t * const DFE_Status_p)
{
    ...
    rc = EIP97_Global_DFE_Status_Get(dev, &dev->Global_IOArea, PE_Number, DFE_Status_p);
        ...
        GlobalControl97_DSE_Status_t * const DSE_Status_p)
{
    ...
    rc = EIP97_Global_DSE_Status_Get(dev, &dev->Global_IOArea, PE_Number, DSE_Status_p);
        ...
        GlobalControl97_Token_Status_t * const Token_Status_p)
{
    ...
    rc = EIP97_Global_EIP96_Token_Status_Get(dev, &dev->Global_IOArea,
                                             PE_Number,
                                             Token_Status_p);
        ...
        GlobalControl97_Context_Status_t * const Context_Status_p)
{
    ...
    rc = EIP97_Global_EIP96_Context_Status_Get(dev, &dev->Global_IOArea,
                                               PE_Number,
                                               Context_Status_p);
        ...
        GlobalControl97_Output_Transfer_Status_t * const OutXfer_Status_p)
{
    ...
    rc = EIP97_Global_EIP96_OutXfer_Status_Get(dev, &dev->Global_IOArea,
                                               PE_Number,
                                               OutXfer_Status_p);
        ...
        GlobalControl97_PRNG_Status_t * const PRNG_Status_p)
{
    ...
    rc = EIP97_Global_EIP96_PRNG_Status_Get(dev, &dev->Global_IOArea,
                                            PE_Number,
                                            PRNG_Status_p);
