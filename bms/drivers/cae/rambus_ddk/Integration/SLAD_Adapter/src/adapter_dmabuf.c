        ...
        DMAResource_Handle_t Handle)
{
    ...
    DMAResource_Translate(dev, Handle, DMARES_DOMAIN_HOST, &HostAddr);
        ...
        const DMAResource_Handle_t Handle2)
{
    ...
    DMAResource_Translate(dev, Handle1, DMARES_DOMAIN_HOST, &AddrPair1);
    DMAResource_Translate(dev, Handle2, DMARES_DOMAIN_HOST, &AddrPair2);
    ...
    if (AddrPair1.Domain == AddrPair2.Domain)
    {
        ...
        const DMAResource_Record_t * const Rec1_p =
                                DMAResource_Handle2RecordPtr(dev, Handle1);
        const DMAResource_Record_t * const Rec2_p =
                                DMAResource_Handle2RecordPtr(dev, Handle2);
        ...
        DMABuf_Handle_t * const Handle_p)
{
    ...
    if( !DMAResource_Alloc(dev, ActualProperties,&AddrPair,&DMAHandle) )
    {
 ...
 */
DMABuf_Status_t
DMABuf_Register(cae_device *dev,
        const DMABuf_Properties_t RequestedProperties,
        void * Buffer_p,
        void * Alternative_p,
        ...
        DMABuf_Handle_t * const Handle_p)
{
    ...
    if( DMAResource_CheckAndRegister(dev, ActualProperties,AddrPair,
            ActualAllocator,&DMAHandle) == 0 )
    {
        if( ActualAllocator == 'C' )
        {
            ...
            DMAResource_AddPair(dev, DMAHandle,AddrPair);
        }
        ...
        DMABuf_Handle_t Handle)
{
    ...
    if( DMAResource_Release(dev, DMAHandle) == 0 )
    {
        return DMABUF_STATUS_OK;
    }
    else
