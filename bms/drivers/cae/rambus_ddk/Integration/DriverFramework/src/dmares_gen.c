    ...
    for(i = 0; i < BankCount; i++)
    {
        ...
        if (HWPAL_DMA_Banks[i].PoolHandle != NULL)
        {
            ...
            if (HWPAL_DMA_Banks[i].fAppsShared)
            {
                ...
                Rec_p = DMAResource_Handle2RecordPtr(dev, HWPAL_DMA_Banks[i].PoolHandle);
                if (Rec_p != NULL)
                    Rec_p->Magic = 0;

                DMAResource_DestroyRecord(dev, HWPAL_DMA_Banks[i].PoolHandle);
            }
            else
#endif // HWPAL_DMARESOURCE_USE_UMDEVXS_DEVICE
            {
                HWPAL_DMAResource_Release(dev, HWPAL_DMA_Banks[i].PoolHandle);
            }
...
DMAResourceLib_DMAPool_Alloc(void)
{
    ...
    for(i = 0; i < BankCount; i++)
    {
...
#endif // !HWPAL_DMARESOURCE_USE_UMDEVXS_DEVICE
        {
            ...
            if (HWPAL_DMAResource_Alloc(dev, PoolProperties,
                                        PoolPropertiesExt,
                                        &PoolAddrPair,
 ...
 */
bool
DMAResource_Init(cae_device *dev)
{
    if (HWPAL_DMAResource_Init(dev) == false)
        return false;
 ...
 */
void
DMAResource_UnInit(cae_device *dev)
{
#ifdef HWPAL_DMARESOURCE_BANKS_ENABLE
    DMAResourceLib_DMAPool_Free();
#endif // HWPAL_DMARESOURCE_BANKS_ENABLE

    HWPAL_DMAResource_UnInit(dev);
}
 ...
 */
int
DMAResource_Alloc(cae_device *dev,
        ...
        DMAResource_Handle_t * const Handle_p)
{
    ...
    if (Bank_p->BankType == HWPAL_DMARESOURCE_BANK_DYNAMIC)
        ...
        return HWPAL_DMAResource_Alloc(dev, RequestedProperties,
                                       PoolPropertiesExt,
                                       AddrPair_p,
                                       Handle_p);
    else
    {   // Handle static banks
        ...
        if (RequestedProperties.Size <= Bank_p->BlockByteCount)
        {
            ...
            Rc = HWPAL_DMAResource_CheckAndRegister(dev, NewProperties,
                                                    AddrPair,
                                                    'R',
                                                    Handle_p);
            if (Rc != 0)
 ...
 */
int
DMAResource_Release(cae_device *dev,
        const DMAResource_Handle_t Handle)
{
    ...
    // Handle non-static banks
    if (Bank_p->BankType == HWPAL_DMARESOURCE_BANK_DYNAMIC)
        return HWPAL_DMAResource_Release(dev, Handle);
    else
    {
 ...
 */
int
DMAResource_CheckAndRegister(cae_device *dev,
        const DMAResource_Properties_t RequestedProperties,
        ...
        DMAResource_Handle_t * const Handle_p)
{
    ...
    // Handle non-static banks
    if (Bank_p->BankType == HWPAL_DMARESOURCE_BANK_DYNAMIC)
        return HWPAL_DMAResource_CheckAndRegister(dev, RequestedProperties,
                                                  AddrPair,
                                                  AllocatorRef,
                                                  Handle_p);
 ...
 */
uint32_t
DMAResource_Read32(cae_device *dev,
        const DMAResource_Handle_t Handle,
        const unsigned int WordOffset)
{
	...
	printf("dma resource read: 0x%lx, 0x%x\n", Address_p + WordOffset, Value);
#endif
        return Value;
    }
}
 ...
 */
void
DMAResource_Write32(cae_device *dev,
        const DMAResource_Handle_t Handle,
        const unsigned int WordOffset,
...
#if CAE_DDK_INFO_DUMP
	printf("dma resource write: 0x%lx, 0x%x\n", Address_p + WordOffset, WriteValue);
#endif
 ...
 */
void
DMAResource_Read32Array(cae_device *dev,
        const DMAResource_Handle_t Handle,
 ...
 */
void
DMAResource_Write32Array(cae_device *dev,
        const DMAResource_Handle_t Handle,
        const unsigned int StartWordOffset,
        const unsigned int WordCount,
 ...
 */
int
DMAResource_SwapEndianness_Set(cae_device *dev,
        const DMAResource_Handle_t Handle,
 ...
 */
int
DMAResource_SwapEndianness_Get(cae_device *dev,
        const DMAResource_Handle_t Handle)
{
    DMAResource_Record_t * Rec_p;
