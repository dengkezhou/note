...
/*
 ...
 Solution:
  ...
  - Handle is a pointer into the dev->Handles_p array. Each entry contains a record
  ...
  - Pointers to these chunks in dev->RecordChunkPtrs_p array. Each record is
    accessed via an index pair (chunk number and index within chunk).
  - list of free locations in dev->Handles_p:  dev->FreeHandles
  - list of free record index pairs     : dev->FreeRecords
 */
...
/* Each DMA handle stores a 32-bit index pair consisting of two 16-bit
   fields to refer to the DMA resource record via dev->RecordChunkPtrs_p.
   dev->RecordChunkPtrs_p is an array of pointers to contiguous arrays
...
#define HWPAL_DMARESOURCE_DCACHE_LINE_SIZE      64//32//L1_CACHE_BYTES
#endif
...
DMAResourceLib_IdxPair2RecordPtr(cae_device *dev, uint32_t IdxPair)
{
    if (IdxPair != HWPAL_INVALID_INDEX &&
        CHUNK_OF(IdxPair) < dev->ChunksCount &&
        RECIDX_OF(IdxPair) < MAX_RECORDS_PER_CHUNK &&
        CHUNK_OF(IdxPair) * MAX_RECORDS_PER_CHUNK +
        RECIDX_OF(IdxPair) < dev->HandlesCount
        )
    {
        return dev->RecordChunkPtrs_p[CHUNK_OF(IdxPair)] + RECIDX_OF(IdxPair);;
    }
    else
    {
        return NULL;
 ...
 */
static inline void
DMAResourceLib_FreeList_Add(cae_device *dev,
        HWPAL_FreeList_t * const List_p,
        uint32_t Nr)
{
...

    {
        ...
        if (WriteIndex_Updated >= dev->HandlesCount)
 ...
 */
static inline DMAResource_Record_t *
DMAResourceLib_InUseHandles_Get(cae_device *dev,
        DMAResourceLib_InUseHandles_Iterator_t * const it)
{
    ...
    do
    {
        if (it->CurIndex >= dev->HandlesCount)
            return NULL;

        Rec_p = DMAResourceLib_IdxPair2RecordPtr(dev, dev->Handles_p[it->CurIndex++]);
...
static inline DMAResource_Record_t *
DMAResourceLib_InUseHandles_First(cae_device *dev,
        DMAResourceLib_InUseHandles_Iterator_t * const it)
{
    it->CurIndex = 0;
    return DMAResourceLib_InUseHandles_Get(dev, it);
}


static inline DMAResource_Record_t *
DMAResourceLib_InUseHandles_Next(cae_device *dev,
        DMAResourceLib_InUseHandles_Iterator_t * const it)
{
    return DMAResourceLib_InUseHandles_Get(dev, it);
}
...
DMAResourceLib_Find_Matching_DMAResource(cae_device *dev,
        ...
        const DMAResource_AddrPair_t AddrPair)
{
    ...
    for (Rec_p = DMAResourceLib_InUseHandles_First(dev, &it);
         Rec_p != NULL;
         Rec_p = DMAResourceLib_InUseHandles_Next(dev, &it))
    {
...
HWPAL_DMAResource_Lock_Alloc(void)
{
   ...
   //     LockSize = 4;
    ...
    HWPAL_SpinLock = HWPAL_DMAResource_MemAlloc(4);
        ...
        DMAResource_Handle_t * const Handle_p)
{
    ...
    // Allocate record
    Handle = DMAResource_CreateRecord(dev);
    if (NULL == Handle)
        return -1;

    Rec_p = DMAResource_Handle2RecordPtr(dev, Handle);
    if (NULL == Rec_p)
    {
        DMAResource_DestroyRecord(dev, Handle);
        return -1;
    }
    ...
    // Allocate DMA resource
    {
        ...
        Device_GetReference(dev, NULL, &DevData);
	    ...
	    //Resource_p = PhysAddr;
            ...
            if (RequestedProperties.fCached)
                IOMem_p = phys_to_virt(PhysAddr);//IOREMAP_CACHE(PhysAddr, n);
            else
                IOMem_p = phys_to_virt(PhysAddr);//ioremap_nocache(PhysAddr, n);
		...
		printf("dma resource alloc: 0x%lx\n", PhysAddr);
#endif
...
#endif // HWPAL_TRACE_DMARESOURCE_BUF
#if CAE_DDK_INFO_DUMP
		printf("dma resource alloc addr: 0x%lx, 0x%lx\n", Handle, DMAAddr);
#endif
        }
        else
        {
...
#endif // HWPAL_TRACE_DMARESOURCE_BUF
#if CAE_DDK_INFO_DUMP
		printf("dma resource alloc bus: 0x%lx, 0x%lx\n", Handle, (DMAAddr - HWPAL_DMARESOURCE_BANK_STATIC_OFFSET));
#endif
...
#endif
#if CAE_DDK_INFO_DUMP
	printf("dma resource alloc final: 0x%lx, 0x%lx, 0x%lx, 0x%lx\n", Handle, DMAAddr, UnalignedAddr_p, AlignedAddr_p);
#endif
        ...
        const DMAResource_Handle_t Handle)
{
        ...
        Rec_p->AllocatorRef == 'R')
    {
        ...
        // Get device reference for this resource
        Device_GetReference(dev, NULL, NULL);
...
#endif
#if CAE_DDK_INFO_DUMP
	printf("dma resource release: 0x%lx, 0x%lx, 0x%lx\n", Handle, DMAAddr, DMAAddr);
#endif
...
HWPAL_DMAResource_Init(cae_device *dev)
{
    ...
    // already initialized?
    if (dev->HandlesCount != 0)
        return false;

    dev->HWPAL_Lock_p = HWPAL_DMAResource_Lock_Alloc();
    if (dev->HWPAL_Lock_p == NULL)
    {
        LOG_CRIT("HWPAL_DMAResource_Init: record lock allocation failed\n");
        return false;
    }

    dev->ChunksCount = (MaxHandles + MAX_RECORDS_PER_CHUNK - 1) /
        MAX_RECORDS_PER_CHUNK;

    if (dev->ChunksCount > 65535 ||
        dev->ChunksCount * sizeof(void *) > KMALLOC_MAX_SIZE)
    {
        LOG_CRIT(
            "HWPAL_DMAResource_Init: "
    ...
    dev->RecordChunkPtrs_p =
            HWPAL_DMAResource_MemAlloc(dev->ChunksCount * sizeof(void *));
    if (dev->RecordChunkPtrs_p)
    {
        // Allocate each of the chunks in the dev->RecordChunkPtrs_p array,
        ...
        memset(
            dev->RecordChunkPtrs_p,
            0,
            dev->ChunksCount * sizeof(DMAResource_Record_t*));
    ...
    { // Too many handles for kmalloc, allocate with get_free_pages.
       ...
       page_cnts = MaxHandles * sizeof(uint32_t) % 4096;
       if (page_cnts == 0) {
               page_cnts = MaxHandles * sizeof(uint32_t) / 4096;
       } else {
               page_cnts = MaxHandles * sizeof(uint32_t) / 4096 + 1;
       }
      ...
      //  dev->Handles_p = (void*) __get_free_pages(GFP_KERNEL, order);
      //  dev->FreeHandles.Nrs_p = (void*) __get_free_pages(GFP_KERNEL, order);
      //  dev->FreeRecords.Nrs_p = (void*) __get_free_pages(GFP_KERNEL, order);
	dev->Handles_p = (void*)malloc(page_cnts * 4096);
	dev->FreeHandles.Nrs_p = (void*)malloc(page_cnts * 4096);
	dev->FreeRecords.Nrs_p = (void*)malloc(page_cnts * 4096);
    }
    else
    {
        dev->Handles_p = HWPAL_DMAResource_MemAlloc(MaxHandles * sizeof(uint32_t));
        dev->FreeHandles.Nrs_p =
                HWPAL_DMAResource_MemAlloc(MaxHandles * sizeof(uint32_t));
        dev->FreeRecords.Nrs_p =
                HWPAL_DMAResource_MemAlloc(MaxHandles * sizeof(uint32_t));
    }

    // if any allocation failed, free the whole lot
    if (dev->RecordChunkPtrs_p == NULL ||
        dev->Handles_p == NULL ||
        dev->FreeHandles.Nrs_p == NULL ||
        dev->FreeRecords.Nrs_p == NULL ||
        AllocFailed)
    {
        LOG_CRIT(
            "HWPAL_DMAResource_Init: "
        ...
        {
            int i;
            for (i = 0; i < dev->ChunksCount; i++)
            {
                if(dev->RecordChunkPtrs_p[i])
                    HWPAL_DMAResource_MemFree(dev->RecordChunkPtrs_p[i]);
            }
            HWPAL_DMAResource_MemFree(dev->RecordChunkPtrs_p);
        }
        ...
        { // Were allocated with get_free pages.
            ...
            if (dev->Handles_p)
                free((void *)dev->Handles_p);

            if (dev->FreeHandles.Nrs_p)
                free((void *)dev->FreeHandles.Nrs_p);

            if (dev->FreeRecords.Nrs_p)
                free((void *)dev->FreeRecords.Nrs_p);
        }
        else
        {
            if (dev->Handles_p)
                HWPAL_DMAResource_MemFree(dev->Handles_p);

            if (dev->FreeHandles.Nrs_p)
                HWPAL_DMAResource_MemFree(dev->FreeHandles.Nrs_p);

            if (dev->FreeRecords.Nrs_p)
                HWPAL_DMAResource_MemFree(dev->FreeRecords.Nrs_p);
        }
        dev->RecordChunkPtrs_p = NULL;
        dev->Handles_p = NULL;
        dev->FreeHandles.Nrs_p = NULL;
        dev->FreeRecords.Nrs_p = NULL;

        if (dev->HWPAL_Lock_p != NULL)
            HWPAL_DMAResource_Lock_Free(dev->HWPAL_Lock_p);
    ...
    // initialize the handles array
    {
        ...
        {
            dev->Handles_p[i] = HWPAL_INVALID_INDEX;
            dev->FreeHandles.Nrs_p[i] = MaxHandles - 1 - i;
            dev->FreeRecords.Nrs_p[i] = COMPOSE_RECNR(chunk,recidx);
            recidx++;
        ...
        dev->FreeHandles.ReadIndex = 0;
        dev->FreeHandles.WriteIndex = 0;

        dev->FreeRecords.ReadIndex = 0;
        dev->FreeRecords.WriteIndex = 0;
    }

    dev->HandlesCount = MaxHandles;
...
HWPAL_DMAResource_UnInit(cae_device *dev)
{
...
#ifdef HWPAL_TRACE_DMARESOURCE_LEAKS
    {
        ...
        for (i = 0; i < dev->HandlesCount; i++)
        {
            uint32_t IdxPair = dev->Handles_p[i];
            ...
            if (IdxPair != HWPAL_INVALID_INDEX)
            {
...

                {
                    DMAResource_AddrPair_t * Pair_p;
                    DMAResource_Record_t * Rec_p =
                        DMAResourceLib_IdxPair2RecordPtr(dev, IdxPair);
    ...
    if (dev->HandlesCount * sizeof(uint32_t) > KMALLOC_MAX_SIZE)
    { // Were allocated with get_free pages.
       // int order = get_order(dev->HandlesCount * sizeof(uint32_t));
...
//
        ...
        else if (AllocatorRef == 'R')
        {
            ...
            ParentRec_p = DMAResourceLib_Find_Matching_DMAResource(dev,
                                                        &ActualProperties,
                                                        AddrPair);
            if (ParentRec_p == NULL)
            {
...
#endif
#if CAE_DDK_INFO_DUMP
	printf("dma resource check: 0x%lx, 0x%lx, 0x%lx\n", Handle, DMAAddr, AddrPair.Address_p);
#endif
...
DMAResource_CreateRecord(cae_device *dev)
{
    unsigned long flags;
    uint32_t HandleNr;
    uint32_t IdxPair = 0;
    ...
    HWPAL_DMAResource_Lock_Acquire(dev->HWPAL_Lock_p, &flags);

    HandleNr = DMAResourceLib_FreeList_Get(dev, &dev->FreeHandles);
    if (HandleNr != HWPAL_INVALID_INDEX)
    {
        IdxPair = DMAResourceLib_FreeList_Get(dev, &dev->FreeRecords);
        if (IdxPair == HWPAL_INVALID_INDEX)
        {
            DMAResourceLib_FreeList_Add(dev, &dev->FreeHandles, HandleNr);
            HandleNr = HWPAL_INVALID_INDEX;
        }
    }

    HWPAL_DMAResource_Lock_Release(dev->HWPAL_Lock_p, &flags);
    ...
    dev->Handles_p[HandleNr] = IdxPair;

    // fill in the handle position
    return dev->Handles_p + HandleNr;
}
 ...
 */
void
DMAResource_DestroyRecord(cae_device *dev,
        const DMAResource_Handle_t Handle)
{
    if (DMAResource_IsValidHandle(dev, Handle))
    {
        uint32_t * p = (uint32_t *)Handle;
        uint32_t IdxPair = *p;

        if (DMAResourceLib_IdxPair2RecordPtr(dev, IdxPair) != NULL)
        {
            unsigned long flags;
            int HandleNr = p - dev->Handles_p;
            ...
            *p = HWPAL_INVALID_INDEX;
            ...
            DMAResourceLib_FreeList_Add(dev, &dev->FreeHandles, HandleNr);
            DMAResourceLib_FreeList_Add(dev, &dev->FreeRecords, IdxPair);
 ...
 */
bool
DMAResource_IsValidHandle(cae_device *dev,
        const DMAResource_Handle_t Handle)
{
    uint32_t * p = (uint32_t *)Handle;

    if (p < dev->Handles_p ||
        p >= dev->Handles_p + dev->HandlesCount)
    {
        return false;
    }
...
DMAResource_Handle2RecordPtr(cae_device *dev,
        const DMAResource_Handle_t Handle)
{
    uint32_t * p = (uint32_t *)Handle;
...
#else
    return dev->RecordChunkPtrs_p[CHUNK_OF(*p)] + RECIDX_OF(*p);
#endif
 ...
 */
void
DMAResource_PreDMA(cae_device *dev,
        const DMAResource_Handle_t Handle,
...
#if CAE_DDK_INFO_DUMP
	printf("dma resource pre: 0x%lx, 0x%x, 0x%x 0x%lx\n", Handle, Offset, NBytes, DMAAddr);
#endif
...
#else
	itcs_dcache_flush_range((phys_addr_t)((uint8_t*)DMAAddr + Offset), size);
 ...
 */
void
DMAResource_PostDMA(cae_device *dev,
        const DMAResource_Handle_t Handle,
        const unsigned int ByteOffset,
        const unsigned int ByteCount)
{
...
#if CAE_DDK_INFO_DUMP
	printf("dma resource post: 0x%lx, 0x%x, 0x%x 0x%lx\n", Handle, Offset, NBytes, DMAAddr);
#endif
	...
	//itcs_dcache_invalid_range((phys_addr_t)((uint8_t*)DMAAddr + Offset), size);
