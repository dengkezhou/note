...
extern Device_Admin_t **Device_Internal_Admin_Get(cae_device *dev);
extern Device_Global_Admin_t *Device_Internal_Admin_Global_Get(cae_device *dev);
extern int Device_Internal_Initialize(cae_device *dev, void * CustomInitData_p);
extern void Device_Internal_UnInitialize(cae_device *dev);
extern Device_Handle_t Device_Internal_Find(cae_device *dev, const char * DeviceName_p, const unsigned int Index);
 ...
 */
static bool
DeviceLib_Device_Exists(cae_device *dev,
        const char * DeviceName_p)
{
    unsigned int i = 0;
    Device_Admin_t ** DevAdmin_pp = Device_Internal_Admin_Get(dev);
 ...
 */
int
Device_Initialize(cae_device *dev,
        void * CustomInitData_p)
{
    ...
    Device_Global_Admin_t * DevGlobalAdmin_p =
                            Device_Internal_Admin_Global_Get(dev);
    ...
    {
        if (DeviceLib_Device_Exists(dev, DevStatAdmin_p[res].DevName))
        {
    ...
    res = (unsigned int)Device_Internal_Initialize(dev, CustomInitData_p);
    if (res != 0)
    {
...
Device_UnInitialize(cae_device *dev)
{
    ...
    if (DevGlobalAdmin_p->fInitialized)
    {
        ...
        Device_Internal_UnInitialize(dev);
        ...
        const char * DeviceName_p)
{
    ...
    for (i = 0; i < DevCount; i++)
        if (DevAdmin_pp[i] &&
            ...
            return Device_Internal_Find(dev, DeviceName_p, i);
        ...
        const Device_Properties_t * const Props_p)
{
    ...
    if (DeviceLib_Device_Exists(dev, Props_p->Name_p))
    {
