...
#define SPI_FLASH_PageProgramQuad  0x38
...
construct_stig_cmd(bool ddr, int lines, bool wr, uint8_t *buf, uint32_t len,
  ...
  case 2:
    ...
    break;
  case 4:
    stigInstr.profile10Instr.instrAddrPhase.ios   = wr ? XSPI_NUMBER_OF_IOS_4 : XSPI_NUMBER_OF_IOS_1;
...
static void nor_program_config_direct_acmd(XSPI_HandleTypeDef *handler){
    ...
    switch(handler->init.line_num){
        case 4:
            ...
            p1config.progSeqAddr.ios = XSPI_NUMBER_OF_IOS_4;
...
static void nor_program_config_direct(XSPI_HandleTypeDef *handler){
    XSPI_ProgramSeqP1Config p1config = { 0 };
...
static void nor_enable_quad_op(XSPI_HandleTypeDef *handler) {
  ...
  sr = nor_read_status(handler, SPI_FLASH_ReadStatusReg);
  ...
  nor_write_status(handler, SPI_FLASH_WriteStatusReg, sr | (1U << 1) | (1U << 6)); /* WEL & QE*/

  wait_busy(handler);

  nor_read_status(handler, SPI_FLASH_ReadStatusReg);
}
...
static int nor_page_program(XSPI_HandleTypeDef *handler, uint8_t *pBuffer, uint32_t addr, uint32_t byte_num)
{
  ...
  case CDNS_XSPI_MODE_DIRECT: {
    ...
    directCmd.transferAddr = addr;
    //nor_program_config_direct(handler);
...
struct xspi_flash_operations f_ops_mxic_nor = {
    .init          = nor_init,
    .read          = nor_read,
    .write         = nor_write,
    .erase         = nor_erase,
