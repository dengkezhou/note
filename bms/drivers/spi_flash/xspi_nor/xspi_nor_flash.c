...
static bool nor_write_protect(XSPI_HandleTypeDef *handler, bool enable) {
  if (enable) {
    XSPI_SetWriteProtect(handler->pD, true);
  } else {
...
struct xspi_flash_operations f_ops_xspi_nor = {
    .init          = nor_init,
    .read          = nor_read,
    .write         = nor_write,
    .erase         = nor_erase,
