...
static void hyper_ram_config(XSPI_HandleTypeDef *handler){
    ...
    g_param.seqType = XSPI_SEQUENCE_TYPE_P2_HYPER_RAM;
    ...
    g_param.pgmPageSize = XSPI_PROGRAM_PAGE_SIZE_128B;
    g_param.rdPageSize = XSPI_READ_PAGE_SIZE_128B;
...
static void hyper_ram_reset(XSPI_HandleTypeDef *handler) {
...
uint32_t hyper_ram_read(XSPI_HandleTypeDef *handler, uint8_t *pBuffer, uint32_t addr,
                  uint32_t byte_num) {
    ...
    case CDNS_XSPI_MODE_STIG: { 
        ...
        stigInstr.profile20Instr.profile20InstrType = XSPI_P20_INSTR_CMD_CODE_SEQ;
        ...
        dataSeqInstr.noOfDummy            = 9;
        #else 
        dataSeqInstr.noOfDummy            = 8;
        #endif
...
bool hyper_ram_erase(XSPI_HandleTypeDef *handler, SPI_FLASH_ERASE_TYPE type,
               uint32_t addr) {
  return true;
}

uint32_t hyper_ram_write(XSPI_HandleTypeDef *handler, uint8_t *pBuffer, uint32_t addr,
                   uint32_t byte_num) {
  switch (handler->work_mode) {
    case CDNS_XSPI_MODE_DIRECT: {
        XSPI_DirectCmd directCmd;
...
bool hyper_ram_write_protect(XSPI_HandleTypeDef *handler, bool enable) {
    return true;
}
...
static int simple_test(XSPI_HandleTypeDef *handler){
  ...
  #if 1
  printf("write start...\n");
  hyper_ram_write(handler, tx, 512, 32);
  printf("write done.\n");
  printf("read hr will be started after 5 seconds.\n");
  udelay(1000*5000);
  #endif

  hyper_ram_read(handler, rx, 512, 32);
  printf("read done.\n");
  printf("tx data:\n");
  for(int i = 0; i < 32; i++){
    printf("0x%x ",tx[i]);
...
#endif
XSPI_HandleTypeDef *hyper_ram_init(XSPI_InitTypeDef *init) {
  ...
  hyper_ram_reset(handler);
  hyper_ram_config(handler);
...
static uint32_t hyper_ram_get_alignment(XSPI_HandleTypeDef *handler) {
    return 0;
}

struct xspi_flash_operations f_ops_xspi_hyper_ram = {
    .init          = hyper_ram_init,
    .read          = hyper_ram_read,
    .write         = hyper_ram_write,
    .erase         = hyper_ram_erase,
    .get_alignment = hyper_ram_get_alignment,
    .write_protect = hyper_ram_write_protect,
    .reset         = hyper_ram_reset};
