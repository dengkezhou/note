...
{
	ids_writel(enable << OVCPCR_UPDATE_PAL, reg_base + OVCPCR);
}

void osd_set_bg_color(uint32_t val)
{
	ids_writel(val, reg_base + OVCBKCOLOR);
}
...
void osd_set_scale_xy(int x, int y)
{
	...
	val = ((x - 1) << OVCWPR_OSDWinX) |
		  ((y - 1) << OVCWPR_OSDWinY);
	ids_writel(val, reg_base + OVCWPR);
}
...
void osd_set_vm_width(int nr, int val)
{
	...
	ids_writel(tmp, reg_base + offset);
}
...
void osd_set_yuvtorgb(struct osd_yuv *yuv)
{
	...
	val = ids_readl(reg_base + OVCOMC);
	val &= ~(0x1 << 31);
	val |= (yuv->enable << 31);
	ids_writel(val, reg_base + OVCOMC);
	if (yuv->enable) {
		val = ids_readl(reg_base + OVCOMC);
		val &= ~(0x1f << 0);
		val &= ~(0x1f << 8);
		...
		ids_writel(val, reg_base + OVCOMC);
		for (i = 0; i < 9; i++)
			ids_writel(yuv->out_coef[i] & 0x7ff, OVCOEF11 + 4 * i + reg_base);
	}
}
...
void osd_set_dma_channel(int nr, int num, struct dma_buffer *dma)
{
	...
	int i;
	printf("func[%s], line:%d, dma_num = %d\n", __func__, __LINE__, num);
	if (num < 1 || num > 4) {
		ids_err("Window dma addr only support [1, 4], num:%d\n",num);
		return;
	}
	...
	for (i = 0; i < num;i++)
		ids_writel(dma[i].phy_addr, reg_base + offset1 + i * 4);
}
...
void osd_set_colorkey(int nr, struct osd_colorkey *colorkey)
{
	...
	val = ids_readl(reg_base + OVCW1CKCR);
	...
	ids_writel(val, reg_base + OVCW1CKCR);
	
	ids_writel(colorkey->colval, reg_base + OVCW1CKR);
}
...
void osd_set_alpha(int nr, struct osd_alpha *alpha)
{
	...
	ids_writel(val, reg_base + OVCW1PCCR);
}
...
void osd_set_scaler_mode(enum scaler_mode mode)
{
	...
	val = ids_readl(reg_base + OVCDCR);
	val &= ~(0x1 << OVCDCR_ScalerMode);
	val |= (mode << OVCDCR_ScalerMode);
	ids_writel(val, reg_base + OVCDCR);
}
...
void osd_set_prescaler_param(struct prescaler_para *para)
{
	...
	ids_writel(val, reg_base + OVCPSCCR);
	val = ((para->x - 1)<< 0) | ((para->y - 1) << 16);
	ids_writel(val, reg_base + OVCPSCPCR);
	ids_writel(para->x, reg_base + OVCPSVSSR);
	if (para->format != OSD_IMAGE_YUV_420SP)
		for (i = 0; i < para->dma->dma_num; i++)
			ids_writel((uintptr_t)(para->dma->buf[i].vir_addr), reg_base + OVCPSB0SAR + i * 4);
	else {
		ids_writel((uintptr_t)(para->dma->buf[0].vir_addr), reg_base + OVCPSB0SAR);
		ids_writel((uintptr_t)(para->dma->buf[1].vir_addr), reg_base + OVCPSCB0SAR);
	}
}
...
void osd_prescaler_bppmode(int mode)
{
	...
	val = ids_readl(reg_base + OVCPSCCR);
	val &= ~(0x1f << 1);
	val |= ((mode & 0x1f) << 1);
	ids_writel(val, reg_base + OVCPSCCR);
}
...
{
	ids_writel((x - 1) | ((y - 1) << 16), reg_base + OVCPSCPCR);
}
...
void osd_set_scaler_input_size(int height, int width)
{
	...
	ids_writel(val, reg_base + SCLIFSR);
}
...
void osd_set_scaler_output_size(int height, int width)
{
	...
	ids_writel(val, reg_base + SCLOFSR);
}
...
void osd_set_scaler_output_offset(int height, int width)
{
	...
	ids_writel(val, reg_base + SCLOFOR);
}
...
void osd_set_pallete(int nr, struct osd_pallete *pallete)
{
	...
	if (pallete->tableLength) {
		...
		ids_writel(val, reg_base + OVCPCR);

		for (i = 0; i < pallete->tableLength; i++)
			ids_writel(pallete->table[i], reg_base + offset + 4 * i);
		val = ids_readl(reg_base + OVCPCR);
		val &= ~(0x1 << OVCPCR_UPDATE_PAL);
		ids_writel(val, reg_base + OVCPCR);
	}
...
void osd_set_pre_pallete(struct osd_pallete *pallete)
{
	...
	if (pallete->tableLength) {
		...
		ids_writel(val, reg_base + OVCPSCCR);
		for (i = 0; i < pallete->tableLength; i++)
			ids_writel(pallete->table[i],  reg_base + 0x1F80 + 4 * i);
	}
}
