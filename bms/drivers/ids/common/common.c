...
static void ids_convert_RGBTOYUV(struct ids_mannual_image *imagecfg)
{
	...
	printf("ids_convert_RGBTOYUV OK\n");
}
...
int ids_draw_image(struct ids_mannual_image *imagecfg, int flags)
{
	...
	if (!imagecfg->mem){
		printf("malloc %d bytes for ids dma\n", imagecfg->width * imagecfg->height * 4);
		imagecfg->mem = (uintptr_t *)malloc(imagecfg->width * imagecfg->height * 4);
	}
	if (!imagecfg->mem) {
	...
	printf("fill framebuffer done\n");
	if (imagecfg->format == OSD_IMAGE_YUV_420SP)
		ids_convert_RGBTOYUV(imagecfg);
	itcs_dcache_flush();
	printf("ids_draw_image ok\n");
	return 0;
}

int ids_read_image(char *path, struct osd_dma *dma)
{
	int fd;
	int ret = -1;

	if (!path || !dma || !dma->buf[0].vir_addr) {
		ids_err("read image err: path %s, mem addr is %p\n",
				path, dma);
		return -1;
	}
	...
	ret = read(fd, dma->buf[0].vir_addr, dma->buf[0].size);
	if (ret != dma->buf[0].size) {
		ids_err("read file err: need %dbyte, get %dbyte\n",
				dma->buf[0].size, ret);
		return -1;
	}
...
void osd_dma_init(struct osd_dma *dma, int flags, int dma_num)
{
	...
	for (i = 0; i < dma_num; i++) {
		...
		dma->buf[i].phy_addr = (uint64_t)dma->buf[i].vir_addr;
		printf("malloc 0x%x bytes for dma in addr %p\n", dma->buf[i].size, dma->buf[i].vir_addr);
	}
}
...
void osd_dma_init_no_malloc(struct osd_dma *dma, int flags, int dma_num, uintptr_t bmp_addr)
{
	int width, height;
	int i;
	
	if (dma_num > 4 || dma_num < 1) {
		ids_err("Framebuffer in [1, 4], val=\n", dma_num);
		return;
	}
	if (flags == CVBS_MODE)
		tvif_get_display(&width, &height);
	...
	for (i = 0; i < dma_num; i++) {
		dma->buf[i].size = width * height * 4;
		dma->buf[i].vir_addr = (void *)bmp_addr;      
		if (!dma->buf[i].vir_addr) {
			ids_err("osd_dma_init: malloc err\n");
			return;
		}
...
static int ids_image_hor_color(struct ids_mannual_image *cfg)
{
	...
	//printf("img hor bar buf size = 0x%x\n", bufsize);
	for (i = 0;i < nheight;i++) 
		for (j = 0;j < bufsize;j++) {
			fillcolor = ids_get_pixcolor(params,
							0xff << (8 * (i % 3)), cfg->alpha);
			if (fillcolor < 0){
				printf("fillcolor = %d, j=%d, i=%d\n", fillcolor, j, i);
				return -1;
			}
			*buf32++ = fillcolor;
		}
...
{
#if 0 //defined(CONFIG_COMPILE_FPGA) || defined(CONFIG_COMPILE_RTL)
    char name[32];
    int len, val;
	uintptr_t addr;
#endif   
   ...
   addr = faddr(cfg->fd);
   memcpy(cfg->mem, (void *)(addr + 0x36), len);
   /*
