...
static uint32_t startHwTransfer(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                                CUSBD_Req const *req);
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-11_4 "const CUSBDMA_DmaTrb* converted
 * to unsigned long, DRV-4773" */
...
static void cusbdAuxBufferInit(CUSBD_PrivateData *dev) {
  ...
  for (epOutIdx = 0U; epOutIdx < CUSBD_NUM_EP_OUT; epOutIdx++) {
    dev->epOutAuxBuffer[epOutIdx].bufferAddr =
        dev->config.epOutAuxBufferCfg[epOutIdx].bufferAddr;
    dev->epOutAuxBuffer[epOutIdx].bufferSize =
        dev->config.epOutAuxBufferCfg[epOutIdx].bufferSize;
    ...
    vDbgMsg(USBSSP_DBG_CUSBD, 1,
            "cusbdAuxBufferInit: epOutIdx(%d) bufferAddr(%x) bufferSize(%x)\n",
            epOutIdx, dev->epOutAuxBuffer[epOutIdx].bufferAddr,
            dev->epOutAuxBuffer[epOutIdx].bufferSize);
  }
  return;
}
 ...
 */
static void cusbdAuxBufferEpSetMaxPktSz(CUSBD_PrivateData *dev,
                                        uint8_t epOutIdx,
 ...
 */
static uint32_t checkAuxMemAvail(CUSBD_EpAuxBuffer *epAuxBuffer,
                                 uint8_t **bufferPtr,
  ...
  if (epAuxBuffer->writeIdx == epAuxBuffer->updateIdx) {
    if ((bufferRemaining >=
         (epAuxBuffer->maxPacketSize + (2U * EPOUT_AUX_BUFFER_HDR_SZ)))) {
      /* we have buffer for transfer */
      ret = CDN_EOK;
    } else if ((epAuxBuffer->writeIdx >= epAuxBuffer->readIdx) &&
               (epAuxBuffer->readIdx >
                (epAuxBuffer->maxPacketSize + EPOUT_AUX_BUFFER_HDR_SZ))) {
 ...
 */
static void cusbdAuxBufferHandleDescMiss(CUSBD_PrivateData *dev,
  ...
  if (ret == CDN_EOK) {
    ...
    xferParams.dmaAddr = getPhyAddrOfU8Ptr(
        &(epAuxBuffer
              ->bufferAddr[epAuxBuffer->writeIdx + EPOUT_AUX_BUFFER_HDR_SZ]));
  ...
  if (ret == CDN_EOK) {
    ...
    /* bytes 0&1 contain next-offset: Update them when transfer done & short
     * packet */
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a "Pass parameter dev with const
 * specifier, DRV-4771" */
/* Note that dev is used to obtain non-const bufferPtr, for update aux-buffer */
static void
cusbdAuxBufferUpdateXfer(CUSBD_PrivateData *dev, const CUSBD_EpPrivate *epp,
  ...
  vDbgMsg(USBSSP_DBG_CUSBD, 1, "EP%d-%s cusbdAuxBufferUpdateXfer xferLen(%d)\n",
          (epp->ep.address & 0xF), (epp->channel->isDirTx) ? "TX" : "RX",
          actualLen);
  ...
  nextOffset += (uint16_t)((actualLen + (EPOUT_AUX_BUFFER_HDR_SZ - 1U)) &
                           (~(EPOUT_AUX_BUFFER_HDR_SZ - 1U)));
...
static uint32_t cusbdAuxBufferIsDataValid(const CUSBD_PrivateData *dev,
  ...
  if (epAuxBuffer->readIdx != epAuxBuffer->updateIdx) {
    dataValid = 1;
    vDbgMsg(USBSSP_DBG_CUSBD, 1,
            "EP%d-%s cusbdAuxBufferIsDataValid dataValid(%d)\n",
            (epp->ep.address & 0xF),
            ((epp->ep.address & 0x80) != 0U) ? "TX" : "RX", dataValid);
  }
  return dataValid;
}
 ...
 */
static uint32_t cusbdAuxBufferDequeueXfer(CUSBD_EpAuxBuffer *epAuxBuffer,
                                          CUSBD_Req *req) {
  ...
  if (reqBufferRem >= length) {
    ...
    auxBufferCopy(&req->buf[req->actual],
                  &bufferPtr[bytesProcessed + EPOUT_AUX_BUFFER_HDR_SZ], length);
    ...
    /* since we didn't consume the entire buffer, store the number of bytes
     * consumed */
...
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a "Pass parameter dev with const
 * specifier, DRV-4772"*/
/* Note that dev is used to obtain non-const epAuxBuffer, for updating
 * aux-buffer */
static uint32_t cusbdAuxBufferDequeue(CUSBD_PrivateData *dev,
  ...
  vDbgMsg(USBSSP_DBG_CUSBD, 1,
          "EP%d-%s cusbdAuxBufferDequeue req->length(%d)\n",
          (epp->ep.address & 0xF),
          ((epp->ep.address & 0x80) != 0U) ? "TX" : "RX", req->length);
  ...
  while (epAuxBuffer->readIdx != epAuxBuffer->updateIdx) {
    ...
    if ((bufferPtr[0] == 0xFFU) && (bufferPtr[1] == 0xFFU) &&
        (epAuxBuffer->updateIdx < epAuxBuffer->readIdx)) {
    ...
    if ((length == 0U) || ((length % epp->ep.maxPacket) != 0U) ||
 ...
 */
static inline CUSBD_EpPrivate *
dmaCompleteCallbackGetEp(CUSBD_PrivateData *dev, uint8_t epNum, uint8_t epDir) {
 ...
 */
static CUSBDMA_DmaTrbChainDesc *
getNextChainDesc(const CUSBDMA_DmaChannel *channel) {
  CUSBDMA_DmaTrbChainDesc *nextChainDesc = NULL;

  if (channel->trbChainDescReadIdx != channel->trbChainDescWriteIdx) {
    nextChainDesc =
        &channel->trbChainDescListHead[channel->trbChainDescReadIdx];
  }
 ...
 */
static void
cusbdmaHandleAuxXferCmpl(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                         const CUSBDMA_DmaTrbChainDesc *nextChainDesc) {
  ...
  if (nextReq != NULL) {
    ...
    isShortPacket =
        ((len == 0U) || ((len % epp->ep.maxPacket) != 0U)) ? 1U : 0U;
 ...
 */
static void
cusbdmaHandleCmplTRBDescEpx(CUSBD_PrivateData *dev,
                            const CUSBDMA_DmaTrbChainDesc *nextChainDesc,
                            CUSBD_EpPrivate *epp) {
 ...
 */
static void cusbdmaHandleCompletedTRBChain(CUSBD_PrivateData *dev,
                                           CUSBD_EpPrivate *epp,
                                           CUSBDMA_DmaChannel *channel) {
  ...
  while (nextChainDesc != NULL) {
    if (nextChainDesc->trbChainState == CUSBDMA_TRB_CHAIN_COMPLETE) {
      /* Check if request is not empty and update number of actually processed
       * bytes */
 ...
 * @param channel
 */
static void cusbdmaProcessDataXferInt(CUSBD_PrivateData *dev,
                                      CUSBDMA_DmaChannel *channel) {
  ...
  if (epp->ep_state == CUSBD_EP_STALLED) {
    (void)CUSBDMA_ChannelHandleStall(&dev->dmaController, channel, 1U,
                                     CUSBD_DEFAULT_TIMEOUT);
  }
}
 ...
 */
static void isrHandleDescMiss(CUSBD_PrivateData *dev,
                              CUSBDMA_DmaChannel *channel) {
  /* call callback if set */
  if (channel->trbBufferEnqueueIdx != channel->trbBufferDequeueIdx) {
    vDbgMsg(USBSSP_DBG_CUSBD, 1,
            "EP%d-%s isrHandleOther: DMARF_EP_DESCMIS: Ignore\n",
            channel->hwUsbEppNum, (channel->isDirTx) ? "TX" : "RX");
  } else if (dev->callbacks.descMissing != NULL) {
    dev->callbacks.descMissing(
        dev, (uint8_t)(channel->hwUsbEppNum | channel->isDirTx));
  } else if ((channel->isDirTx == 0U) && (channel->hwUsbEppNum != 0U)) {
    CUSBD_EpPrivate *epp =
        dmaCompleteCallbackGetEp(dev, channel->hwUsbEppNum, channel->isDirTx);
 ...
 * @param channel: pointer to DMA channel
 */
static void isrHandlePrime(CUSBD_PrivateData *dev,
 ...
 * @param channel: pointer to DMA channel
 */
static void isrHandleMdExit(CUSBD_PrivateData *dev,
                            CUSBDMA_DmaChannel *channel) {
  ...
  if (channel->isDirTx == 0U) {
    ...
    /* update request only when all data has been transferred for current
     * request*/
    if (tdl == 0U) {
      /* Handle tdl = 0 */
    ...
    vDbgMsg(USBSSP_DBG_CUSBD, 1,
            "EP%d-%s MD_EXIT Ignoring for IN transfers \r\n",
            channel->hwUsbEppNum, (channel->isDirTx ? "TX" : "RX"));
  }
}
 ...
 */
static void isrHandleOther(CUSBD_PrivateData *dev, CUSBDMA_DmaChannel *channel,
                           uint32_t epSts) {
   ...
   * If PRIME & MD_EXIT came together this would cause a redundant ERDY to be
   * sent to HOST however sending redundant ERDY should not be an issue */
 ...
 * @param epSts Endpoint status
 */
static void isrHandle(CUSBD_PrivateData *dev, CUSBDMA_DmaChannel *channel,
                      uint32_t epSts) {
  ...
  /* check IOC and ISP */
  if (((epSts & DMARF_EP_IOC) != 0U) || ((epSts & DMARF_EP_ISP) != 0U) ||
      ((epSts & DMARF_EP_IOT) != 0U)) {
 ...
 * @return next endpoint request, if NULL it means that next request does not
 * exist
 */
...
static CH9_UsbSpeed getActualSpeed(CUSBD_PrivateData *dev) {
  ...
  /*super plus*/
  case 5:
    actualSpeed = CH9_USB_SPEED_SUPER_PLUS;
    break;
  /* unknown*/
  default:
...
static uint32_t startHwTransfer(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
  ...
  uint32_t numOfbytes      = (bytesToTransfer > TD_SING_MAX_TRB_DATA_SIZE)
                            ? TD_SING_MAX_TRB_DATA_SIZE
                            : bytesToTransfer;
 ...
 */
static uint32_t ep0transfer(CUSBD_PrivateData *dev, uint8_t dir,
                            const CUSBD_Req *req, uint8_t erdy) {

  uint32_t ret = CDN_EOK;
  CUSBDMA_DmaChannel *channel =
      (dir > 0U) ? dev->ep0DmaChannelIn : dev->ep0DmaChannelOut;
  CUSBDMA_DmaTransferParam trParams;
  ...
  if ((dir != 0U) || (programDma != 0U)) {
    ...
    trParams.len     = (req->length < dev->ep0.ep.maxPacket)
                       ? req->length
    ...
    vDbgMsg(USBSSP_DBG_CUSBD, DBG_HIVERB,
            "Programming EP0 transfer for length %d\n", trParams.len);
  ...
  if (ret != CDN_EOK) {
    ...
    if (erdy != 0U) {
      /* send ERDY packet */
      CPS_UncachedWrite32(&dev->reg->USBR_EP_CMD,
                          EP_CMD_ERDY); /* OUT data phase */
    }
  }
  return ret;
}
 ...
 */
static void epEnableInterrupt(CUSBD_PrivateData *dev, uint8_t epNum,
                              uint8_t epDir) {
 ...
 * @return register
 */
static uint32_t constructEpCfgReg(uint8_t epType, uint16_t maxPacketSize,
                                  uint8_t epBuffering, uint8_t mult,
                                  uint8_t maxBurst) {
  uint32_t reg = 0;

  SET_EP_CONF_ENABLE(&reg);         /* Enable end point */
  SET_EP_CONF_EPTYPE(&reg, epType); /* set ep type */
  SET_EP_CONF_MAXPKTSIZE(&reg,
                         le16ToCpu(maxPacketSize)); /* set max packet size */
 ...
 */
static void epEnableHw(CUSBD_PrivateData *dev, CUSBD_Ep *ep,
                       const uint8_t *desc,
  ...
  uint32_t irqEnFlags =
      EP_STS_EN_TRBERREN | EP_STS_EN_ISOERREN | EP_STS_EN_DESCMISEN;
  ...
  reg = constructEpCfgReg(epType, channelParams->wMaxPacketSize, epBuffering,
                          mult, ep->maxburst);
  ...
  if (dev->device.speed >= CH9_USB_SPEED_SUPER) {
    ...
    if ((desc[7] == CH9_USB_DS_SS_USB_EP_COMPANION) &&
      ...
      /* need some cleanup - 'maybe' - currently there is redundancy, but this
       * works for ep-in we use IOT for ep-out we use MD_EXIT + TDL */
      if (ep->compDesc[3] > 0U) {
        SET_EP_CONF_STREAM_EN(&reg);
        reg |= 0x10U; /* set TDL_CHK in ep_cfg */
        reg |= 0x20U; /* set SID_CHK in ep_cfg */
        ep->maxStreams = ep->compDesc[3];
        irqEnFlags |= DMARF_EP_IOT | DMARF_EP_PRIME | DMARF_EP_SIDERR |
                      DMARF_EP_MD_EXIT | DMARF_EP_STREAMR;
      }
    } else {
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
              "Endpoint companion descriptor not found for ep: %02X\n",
              ep->address);
    }
  }
 ...
 * @param maxPacketSize max packet size
 */
static uint32_t epEnableEx(CUSBD_PrivateData *dev, CUSBD_Ep *ep,
                           const uint8_t *desc, uint16_t maxPacketSize) {
  ...
  ret = CUSBDMA_ChannelAlloc(&(dev->dmaController), &epp->channel,
                             &channelParams);
  ...
  if (ret == CDN_EOK) {
    ...
    vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
            "Could not allocate DMA channel EP%d-%s", epNum,
            (epDir != 0U) ? "TX" : "RX");
  }
  ...
  if ((desc[7] == CH9_USB_DS_SS_USB_EP_COMPANION) &&
    ...
    *maxBurstSize = desc[9];
    ...
    if (epDescType == CH9_USB_EP_INTERRUPT) {
      if (*maxBurstSize > 2U) {
        vDbgMsg(USBSSP_DBG_CUSBD, DBG_HIVERB,
                "WARNING: Limiting SS interrupt-ep maxBurstSize(%d) to 2 \n",
                *maxBurstSize);
        *maxBurstSize = 2;
      }
    }
 ...
 */
static uint8_t getMult(const uint8_t *epDesc, CH9_UsbSpeed actualSpeed,
                       uint8_t maxburst) {
  ...
  if ((actualSpeed >= CH9_USB_SPEED_SUPER) &&
      (epDescType == CH9_USB_EP_ISOCHRONOUS) && (maxburst != 0U)) {
 ...
 * @return CDN_EOK on success, error otherwise
 */
static uint32_t epEnable(CUSBD_PrivateData *pD, CUSBD_Ep *ep,
                         const uint8_t *desc) {
  ...
  if (desc[1] != CH9_USB_DT_ENDPOINT) {
    vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
            "Invalid endpoint descriptor on ep: %02X\n", ep->address);
    ret = CDN_EINVAL;
  }
...
static uint32_t epDisable(CUSBD_PrivateData *pD, CUSBD_Ep *ep) {
  ...
  if (ret == CDN_EOK) {
    ...
    if (ret != CDN_EOK) {
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
              "Could not release DMA channel on ep:%02X \n", ep->address);
    }
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t reqQueueRunHw(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                              CUSBD_Req *req) {
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t handleSwEpOutReq(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                                 CUSBD_Req *req) {
  uint32_t ret           = CDN_EOK;
  uint32_t isShortPacket = 0U;
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t handleSwEpInReq(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                                CUSBD_Req *req) {
  uint32_t ret = CDN_EOK;
...
static uint32_t handleEp0Req(CUSBD_PrivateData *dev, CUSBD_Req *req) {
  ...
  if (dev->ep0NextState == CH9_EP0_STATUS_PHASE) {
    ...
    CPS_UncachedWrite32(&dev->reg->USBR_EP_CMD,
                        EP_CMD_REQ_CMPL | EP_CMD_ERDY); /* status phase */
    ...
    if (dev->ep0NextState != CH9_EP0_DATA_PHASE) {
      ...
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_FYI,
              "Processing EP0 req in data phase %d \n", 0);
      ep0Req.dma    = req->dma;
      ep0Req.length = req->length;
      ep0Req.zero   = req->zero;
      ret           = ep0transfer(dev, dev->ep0DataDirFlag, &ep0Req,
                        (dev->ep0DataDirFlag > 0U) ? 0U : 1U);
      dev->request  = req;
    }
  }
  return ret;
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t handleSwEpReq(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                              CUSBD_Req *req) {
  uint32_t ret = CDN_EOK;
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t handleStreamEpReq(CUSBD_PrivateData *dev, CUSBD_EpPrivate *epp,
                                  CUSBD_Req *req) {
  uint32_t ret               = CDN_EOK;
  ...
  vDbgMsg(
      USBSSP_DBG_CUSBD, DBG_FYI,
      "EP%d-%s size(%d) streamId(%d) reqBuf(0x%X) lastRequestPending(%d)\r\n",
   ...
   * The streamID of the lastReq is same as the current req AND last req is also
   * queued in */
  if ((lastReq == NULL) ||
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t reqQueue(CUSBD_PrivateData *dev, const CUSBD_Ep *ep,
                         CUSBD_Req *req) {
  ...
  vDbgMsg(USBSSP_DBG_CUSBD, DBG_FYI,
          "EP%d-%s req(0x%08X) buffer(0x%08X) size(0x%08X) \n",
          (ep->address & 0xF), ((ep->address & 0xF0) == 0U) ? "RX" : "TX",
          (uintptr_t)req, (uintptr_t)req->buf, req->length);
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t epSetHalt(CUSBD_PrivateData *pD, const CUSBD_Ep *ep,
                          uint8_t value) {
  ...
  if (ret == CDN_EOK) {
    if (value > 0U) {
      /* set stall */
      ret = CUSBDMA_ChannelHandleStall(&dev->dmaController, epp->channel, 1U,
                                       CUSBD_DEFAULT_TIMEOUT);
      ...
      /* clear stall */
      epp->wedgeFlag = 0;
      ret = CUSBDMA_ChannelHandleStall(&dev->dmaController, epp->channel, 0U,
                                       CUSBD_DEFAULT_TIMEOUT);
      epp->ep_state = CUSBD_EP_ENABLED;
...
static uint32_t epSetWedge(CUSBD_PrivateData *pD, const CUSBD_Ep *ep) {
  ...
  vDbgMsg(USBSSP_DBG_CUSBD, DBG_FYI, "EP%d-%s epSetWedge", (ep->address & 0xF),
          ((ep->address & 0x80) != 0U) ? "TX" : "RX");
...
static void initDevHw(CUSBD_PrivateData *dev) {
  ...
  if (dev->config.forcedUsbMode == 1U) {
    ...
    printf("****************USB forced to 3.0****************\n");
    /* Needed for MISRA compliance */
  }

  /* configure device */
  CPS_UncachedWrite32(&reg->USBR_CONF,
                      USB_CONF_L1EN | dmult | usbForceDisconnect);
  ...
  /* enable interrupts */
  CPS_UncachedWrite32(&dev->reg->USBR_IEN,
                      USB_IEN_UWRESIEN | USB_IEN_UHRESIEN | USB_IEN_DISIEN |
                          USB_IEN_CONIEN | USB_IEN_CON2I | USB_IEN_U1ENTIEN |
 ...
 * @return status
 */
static uint32_t initSwEp(CUSBD_PrivateData *dev, const CUSBD_EpConfig *endpoint,
                         uint8_t dir) {
  ...
  for (i = 0U; i < 15U; i++) {
    if (ep[i].bufferingValue > 0U) {
      ...
      epPriv->ep.address =
          ((dir > 0U) ? CH9_USB_EP_DIR_IN : 0U) | (uint8_t)(i + 1U);
      /* set MaxPacketSize */
...
static void updateReqAlloc(CUSBD_PrivateData *dev, uint16_t size) {
  ...
  if (ret != CDN_EOK) {
    vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
            "Could not reply to GET STATUS command\n", 0);
  }
}
...
static uint32_t getStatus(CUSBD_PrivateData *dev, CH9_UsbSetup *ctrl) {
  ...
  uint32_t ret  = CDN_EOK;
  /* parasoft-begin-suppress MISRA2012-RULE-11_4 " uintptr_t converted to
   * 'uint32_t*', DRV-4385" */
  ...
  case CH9_USB_REQ_RECIPIENT_DEVICE:
    dev->status_value = cpuToLe16((uint16_t)dev->u2_value |
                                  (uint16_t)dev->u1_value | (uint16_t)1U);
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t handleFeatureRecDevTest(CUSBD_PrivateData *dev,
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t handleFeature(CUSBD_PrivateData *dev, CH9_UsbSetup const *ctrl,
                              uint8_t set) {
...
static uint32_t setAddress(CUSBD_PrivateData *dev, CH9_UsbSetup *ctrl) {
  ...
  if (ret == CDN_EOK) {
    if (state == CH9_USB_STATE_CONFIGURED) {
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
              "Trying to set address when configured %c\n", ' ');
      ret = CDN_EINVAL;
    }
  }
  ...
  if (ret == CDN_EOK) {
    ...
    CPS_UncachedWrite32(&dev->reg->USBR_CMD,
                        (((uint32_t)ctrl->wValue & 0x007FU) << 1) |
                            0x00000001U);
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t setConfigStateAddressed(CUSBD_PrivateData *dev,
                                        CH9_UsbSetup *ctrl, uint8_t confValue) {
 ...
 * @return CDN_EOK for success, error code elsewhere
 */
static uint32_t setIsochDelay(CUSBD_PrivateData *dev,
                              CH9_UsbSetup const *ctrl) {
...
static uint32_t setupDevStand(CUSBD_PrivateData *dev, CH9_UsbSetup *ctrl) {
  ...
  case CH9_USB_REQ_SET_ADDRESS:
    printf("****************setupDevStand CH9_USB_REQ_SET_ADDRESS****************\n");
...
static void handleEp0IrqStatus(CUSBD_PrivateData *dev) {
  if (dev->ep0NextState == CH9_EP0_STATUS_PHASE) {
    ...
    if (dev->request != NULL) {
      if ((dev->request->length == 0U) &&
          (dev->request->status == CDN_EINPROGRESS)) {
...
uint32_t CUSBD_Probe(const CUSBD_Config *config, CUSBD_SysReq *sysReqCusbd) {
  ...
  if (ret == CDN_EOK) {
    ...
    if ((config->didRegPtr == 0U) || (config->ridRegPtr == 0U)) {
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
              "Warning! DID, RID register pointers are not set.\n", 0);
    } else {
      /* parasoft-begin-suppress MISRA2012-RULE-11_4 "uintptr_t converted to
       * 'uint32_t *', DRV-4273" */
      ...
      if ((did != CUSBD_DID_VAL) && (rid != CUSBD_RID_VAL)) {
        ret = CDN_EPERM;
      }
    }
  }
...
static uint32_t ramInit(CUSBD_PrivateData *pD) {
  ...
  /*
      ...
      - It must be performed before any Endpoint configurations ( any EP_XXX
     register)
  ...
  ret = waitUntilBitSet(&(dev->reg->USBR_STS), USB_STS_IN_RST);
    ...
    uint32_t base = pD->config.regBase - 0x20000;
    printf("speed up reg base: 0x%x\n", base);

    writel(0x00000010,base + 0x000201d8);
    writel(0x000000e1,base + 0x000201e4);
    writel(0x00001800,base + 0x000201e8);
    writel(0x00000300,base + 0x000201e0);
    writel(0x00000039,base + 0x00020220);
    writel(0x00000039,base + 0x00020224);
    writel(0x00000039,base + 0x00020228);
    writel(0x00000111,base + 0x000201f8);
    writel(0x00000001,base + 0x00020100);
    writel(0x00000001,base + 0x00020200);
    writel(0x4000000d,base + 0x0002010c);
    writel(0x8000000b,base + 0x00020110);
    writel(0x4000000d,base + 0x00020114);
    writel(0x400001e1,base + 0x00020118);
    writel(0x4000001f,base + 0x0002011c);
    writel(0x40000003,base + 0x00020120);
    writel(0x4000001f,base + 0x00020124);
    writel(0x40000065,base + 0x00020128);
    writel(0x40000065,base + 0x0002012c);
    writel(0x40000003,base + 0x00020130);
    writel(0x4000000d,base + 0x00020134);
    writel(0x40000003,base + 0x00020138);
    writel(0x40000004,base + 0x0002013c);
    writel(0x4000005b,base + 0x00020140);
    writel(0x8000000b,base + 0x00020144);
    writel(0x4000000b,base + 0x00020148);
    writel(0x40000081,base + 0x0002014c);
    writel(0x80000003,base + 0x00020158);
    writel(0x0007a120,base + 0x0002015c);
    writel(0x004c4b40,base + 0x00020160);
    writel(0x0000f424,base + 0x000201b4);
    writel(0x0000f424,base + 0x00020204);
    writel(0x0001e848,base + 0x000201b8);
    writel(0x000000fa,base + 0x000201c4);
    writel(0x000001f4,base + 0x000201c8);
    writel(0x000000fa,base + 0x000201cc);
    writel(0x000000c8,base + 0x00020210);
    writel(0x00000020,base + 0x000201d4);

#endif

  if (ret == CDN_EOK) {
    /* set USB_PWR.FAST_REG_ACCESS bit (to ensure that USB clock is running even
     * without VBUS) */
  ...
  if (ret == CDN_EOK) {
    /* set BUF_ADDR register (it addresses individual words, not Bytes) to max
     * address for attached memory . */
    /* In case of TI we have 4 * 3328 of 32bit words, so max address is 13312
     * (0x3400). */
    /* So we need to write 0x3400 to BUF_ADDR */
    
#if CONFIG_COMPILE_RTL
    //change by jason
    CPS_UncachedWrite32(&dev->reg->USBR_BUF_ADDR, 0x000011FF);
#else
    CPS_UncachedWrite32(&dev->reg->USBR_BUF_ADDR, 0x000033FF);
#endif

    /* BUF_DATA can be left as 0x00000000 (RSTV) then whole memory will be
     * initialized to 0 */
    CPS_UncachedWrite32(&dev->reg->USBR_BUF_DATA, 0x00000000);

    /* set BUF_CTRL.BUF_CMD_IWD (Incremental write down) bit + BUF_CMD_SET (one
     * write). */
    /* This will start internal operation of writing all memory cells (starting
     * from BUFF_ADDR down to 0) */
    /* with value prepared in BUF_DATA. For TI memory initialization should take
     * about 27 us. */
    CPS_UncachedWrite32(&dev->reg->USBR_BUF_CTRL,
                        BUF_CTRL_BUF_CMD_SET | BUF_CTRL_BUF_CMD_IWD);

    /* wait until BUF_CTRL.BUF_CMD_STS is '0' it means that memory
 ...
 */
static inline uint32_t initConfigDMA(CUSBD_PrivateData *dev,
                                     CUSBDMA_Config *dmaConfig) {
  ...
  if (dev->deviceVersion < DEV_VER_V3) {
    ...
    dmaConfig->dmaModeTx =
        (uint16_t)((dev->config.dmultEnabled >> 16U) & 0xFFFFU);
  }
 ...
 */
static inline uint32_t initConfigDev(CUSBD_PrivateData *dev,
                                     CUSBDMA_Config *dmaConfig) {
...
static inline uint32_t initEp0(CUSBD_PrivateData *dev) {
  ...
  channelParams.epIrqConfig    = EP_STS_EN_SETUPEN | EP_STS_EN_DESCMISEN |
                              EP_STS_EN_TRBERREN | EP_STS_EN_OUTSMMEN;
  ...
  ret = CUSBDMA_ChannelAlloc(&(dev->dmaController), &dev->ep0DmaChannelIn,
                             &channelParams);
  if (ret != CDN_EOK) {
  ...
  if (ret == CDN_EOK) {
    ...
    ret = CUSBDMA_ChannelAlloc(&(dev->dmaController), &dev->ep0DmaChannelOut,
                               &channelParams);
    if (ret != CDN_EOK) {
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
              "Could not allocate EP0-OUT channel\n", 0);
    }
  }
  return ret;
}
...
static inline uint32_t initEp(CUSBD_PrivateData *dev) {
  ...
  if (ret == CDN_EOK) {
    ...
    if (ret != CDN_EOK) {
      vDbgMsg(USBSSP_DBG_CUSBD, DBG_CRIT,
              "Could not initialize OUT endpoints\n", 0);
    }
  }
 ...
 */
uint32_t CUSBD_Init(CUSBD_PrivateData *pD, const CUSBD_Config *config,
 ...
 */
static inline void handleIsrDevRS(CUSBD_PrivateData *dev, uint32_t usb_ists,
                                  uint8_t *exitFlag) {

  vDbgMsg(USBSSP_DBG_CUSBD, DBG_FYI,
          "handleIsrDevRS called with usb_ists: %x\n", usb_ists);
  /* set speed field */
  ...
  CPS_UncachedWrite32(&dev->reg->USBR_ISTS,
                      USB_ISTS_DISI | USB_ISTS_UWRESI | USB_ISTS_UHRESI |
                          USB_ISTS_DIS2I | USB_ISTS_U2RESI);
 ...
 */
static inline void handleIsrDevOtherU3(CUSBD_PrivateData *dev,
                                       uint32_t usb_ists, uint8_t *exitFlag) {
 ...
 * @param exitFlag pointer to exitFlag
 */
static inline void handleIsrDevOtherU2(CUSBD_PrivateData *dev,
                                       uint32_t usb_ists, uint8_t *exitFlag) {
 ...
 * @param exitFlag pointer to exitFlag
 */
static inline void handleIsrDevOtherU1(CUSBD_PrivateData *dev,
                                       uint32_t usb_ists, uint8_t *exitFlag) {
 ...
 * @param exitFlag pointer to exitFlag
 */
static inline void handleIsrDevOther(CUSBD_PrivateData *dev, uint32_t usb_ists,
                                     uint8_t *exitFlag) {
 ...
 */
static inline void handleIsrDev(CUSBD_PrivateData *dev, uint32_t usb_ists,
                                uint8_t *exitFlag) {
  ...
  if ((((usb_ists & USB_ISTS_CONI) > 0U) ||
       ((usb_ists & USB_ISTS_CON2I) > 0U)) &&
      (*exitFlag == 0U)) {
    ...
    (void)CUSBDMA_ChannelSetMaxPktSz(&dev->dmaController, dev->ep0DmaChannelIn,
                                     dev->ep0.ep.maxPacket);
    (void)CUSBDMA_ChannelSetMaxPktSz(&dev->dmaController, dev->ep0DmaChannelIn,
                                     dev->ep0.ep.maxPacket);
 ...
 * @return CDN_EOK on success, error otherwise
 */
static inline uint32_t isrHandleEp0(CUSBD_PrivateData *dev, uint32_t ep_ists,
                                    uint8_t *exitFlag) {
  uint32_t ret = CDN_EOK;
...
uint32_t CUSBD_Isr(CUSBD_PrivateData *pD) {
  ...
  if (ret == CDN_EOK) {
    ...
    /* check if interrupt generated by default endpoint on both directions (ss
     * speed) */
