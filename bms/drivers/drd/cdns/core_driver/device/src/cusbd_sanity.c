...
uint32_t CUSBD_ConfigSF(const CUSBD_Config *obj) {
  ...
  if (obj == NULL) {
    ...
    if ((obj->dmaInterfaceWidth != CUSBD_DMA_64_WIDTH)) {
      ret = CDN_EINVAL;
    }
  }
...
uint32_t CUSBD_PrivateDataSF(const CUSBD_PrivateData *obj) {
  ...
  if (obj == NULL) {
    ...
    if ((obj->ep0NextState != CH9_EP0_UNCONNECTED) &&
    ...
    for (idx_ep_in_container = 0; idx_ep_in_container < 16U;
         idx_ep_in_container++) {
      if (CUSBD_EpPrivateSF(&obj->ep_in_container[idx_ep_in_container]) ==
          CDN_EINVAL) {
        ret = CDN_EINVAL;
      }
    }
    ...
    for (idx_ep_out_container = 0; idx_ep_out_container < 16U;
         idx_ep_out_container++) {
      if (CUSBD_EpPrivateSF(&obj->ep_out_container[idx_ep_out_container]) ==
          CDN_EINVAL) {
        ret = CDN_EINVAL;
      }
    }
...
uint32_t CUSBD_DevSF(const CUSBD_Dev *obj) {
  ...
  if (obj == NULL) {
    ...
    if ((obj->speed != CH9_USB_SPEED_UNKNOWN) &&
    ...
    if ((obj->maxSpeed != CH9_USB_SPEED_UNKNOWN) &&
    ...
    if ((obj->state != CH9_USB_STATE_NONE) &&
...
uint32_t CUSBD_EpPrivateSF(const CUSBD_EpPrivate *obj) {
  ...
  if (obj == NULL) {
    ...
    if ((obj->ep_state != CUSBD_EP_DISABLED) &&
 ...
 * @return CDN_EINVAL invalid parameters
 */
uint32_t CUSBD_SanityFunction8(const CUSBD_PrivateData *pD,
                               const CUSBD_Ep *ep) {
  /* Declaring return variable */
  uint32_t ret = 0;
