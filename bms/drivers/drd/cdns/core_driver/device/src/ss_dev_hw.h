...
static inline void SET_EP_CONF_MAXPKTSIZE(uint32_t *reg, uint16_t maxpcksize) {
  (*reg) |= (((uint32_t)(maxpcksize) & (uint32_t)0x00FFU) << 16) |
            (((uint32_t)(maxpcksize) & (uint32_t)0x1F00U) << 16);
}
 ...
 */
static inline uint32_t SET_DMA_AXI_CTRL_MARPROT(uint32_t reg,
                                                uint32_t marprot) {
  uint32_t regval = reg & (~DMA_AXI_CTRL_MARPROT_MASK);
  regval |=
      ((marprot << DMA_AXI_CTRL_MARPROT_OFFST) & DMA_AXI_CTRL_MARPROT_MASK);
  return regval;
}
 ...
 * @return: New value of DMA_AXI_CTRL reg
 */
static inline uint32_t SET_DMA_AXI_CTRL_MAWPROT(uint32_t reg,
                                                uint32_t mawprot) {
  uint32_t regval = reg & (~DMA_AXI_CTRL_MAWPROT_MASK);
  regval |=
      ((mawprot << DMA_AXI_CTRL_MAWPROT_OFFST) & DMA_AXI_CTRL_MAWPROT_MASK);
  return regval;
}
#endif
