...
static void channelUpdateState(const CUSBDMA_DmaController *pD,
 ...
 */
static uint32_t waitForBitCleared(uint32_t *reg, uint32_t bit,
                                  uint32_t timeout) {
 ...
 */
static inline void BUILD_NORMAL_TRB_NO_IOC(CUSBDMA_DmaTrb *trb,
                                           uint32_t data_ptr,
                                           uint32_t data_size,
                                           uint16_t stream_id,
                                           uint32_t cycleBit) {
  ...
  trb->ctrl    = cpuToLe32(((uint32_t)(stream_id) << 16U) |
                        (uint32_t)(TD_TYPE_NORMAL | cycleBit));
}
 ...
 * @param stream_id stream ID
 */
static inline void BUILD_NORMAL_TRB_NO_IOC_CHAIN(CUSBDMA_DmaTrb *trb,
                                                 uint32_t data_ptr,
                                                 uint32_t data_size,
                                                 uint16_t stream_id,
                                                 uint32_t cycleBit) {
  ...
  trb->dmaSize = cpuToLe32(data_size | (uint32_t)PRECISE_BURST_LENGTH);
  trb->ctrl    = cpuToLe32(((uint32_t)(stream_id) << 16U) |
                        (uint32_t)(TD_TYPE_NORMAL | cycleBit | TDF_CHAIN_BIT));
}
 ...
 * @param stream_id stream ID
 */
static inline void BUILD_NORMAL_TRB(CUSBDMA_DmaTrb *trb, uint32_t data_ptr,
                                    uint32_t data_size, uint16_t stream_id,
                                    uint32_t cycleBit) {
  trb->dmaAddr = cpuToLe32(data_ptr);
  trb->dmaSize = cpuToLe32((data_size) | (uint32_t)PRECISE_BURST_LENGTH);
  trb->ctrl =
      cpuToLe32(((uint32_t)(stream_id) << 16U) |
                (uint32_t)(TD_TYPE_NORMAL | cycleBit | TDF_INT_ON_COMPLECTION |
                           TDF_INT_ON_SHORT_PACKET));
}
 ...
 * @param toggle cycle
 */
static inline void BUILD_LINK_TRB(CUSBDMA_DmaTrb *trb, uint32_t target_ptr,
                                  uint32_t cycleBit, uint32_t toggleCycle) {
 ...
 */
static inline uintptr_t getPhyAddrOfTRBPtr(const CUSBDMA_DmaChannel *channel,
                                           const CUSBDMA_DmaTrb *trb) {
  /* parasoft-begin-suppress MISRA2012-RULE-11_4 "const CUSBDMA_DmaTrb*
   * converted to unsigned long, DRV-5041" */
  uintptr_t trb_vaddr = ((uintptr_t)trb);
  /* parasoft-end-suppress MISRA2012-RULE-11_4 */
  /* parasoft-begin-suppress MISRA2012-RULE-11_4 "CUSBDMA_DmaTrb* converted to
   * unsigned long, DRV-5042" */
  ...
  vDbgMsg(DBG_DMA_BASIC_MSG, 1,
          "EP%d-%s trb_vaddr(0x%08x) trb_phyaddr(0x%08x)\n",
          channel->hwUsbEppNum, (channel->isDirTx) ? "TX" : "RX", trb_vaddr,
          trb_phyaddr);
 ...
 */
static uint32_t getNumDataTRBs(uint8_t dir, uint8_t zero, uint32_t dataLen,
                               uint32_t maxTrbLen) {
  /* Calculate number of TRBs including short TRB */
  uint32_t numTRBs =
      (maxTrbLen != 0U) ? (((dataLen + maxTrbLen) - 1U) / maxTrbLen) : 0U;
...
static uint32_t getActualNumTRBs(const CUSBDMA_DmaChannel *channel,
  ...
  uint32_t numOfTrbs    = getNumDataTRBs(channel->isDirTx, trParams->zlp,
                                      trParams->len, channel->maxTrbLen);
 ...
 * @param actualNumTRBs Number of TRBs to allocate. Should be always less than
 * TRB buffer size
 * @return Index of allocation if success OR (-1) if failure
 */
static int32_t channelAllocTRBs(CUSBDMA_DmaChannel *channel,
                                uint32_t actualNumTRBs) {
  ...
  if ((ringSize + actualNumTRBs) < bufferSize) {
    ...
    vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
            "Error: EP%d-%s TRB trbWrIdx(&d) trbRdIdx(%d) actualNumTRBs(%d) "
            "trbBufferSize(%d)\n",
            channel->hwUsbEppNum, (channel->isDirTx) ? "TX" : "RX", trbWrIdx,
            trbRdIdx, actualNumTRBs, bufferSize);
  }
  return trbStartIdx;
}
...
static int32_t allocTRBChainDesc(CUSBDMA_DmaChannel *channel,
  ...
  if (nextWrIdx != rdIdx) {
    ...
    if (trbStartIdx >= 0) {
      CUSBDMA_DmaTrbChainDesc *trbChainDesc =
          &channel->trbChainDescListHead[wrIdx];
      ...
      trbChainDesc->end =
          (uint16_t)((((uint32_t)trbStartIdx) + (actualNumTRBs - 1U)) %
                     (channel->trbBufferSize));
    ...
    vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
            "Error: EP%d-%s TRBChainDesc nextWrIdx(%d) rdIdx(%d) "
            "trbChainDescBufferSz(%d)\n",
            channel->hwUsbEppNum, (channel->isDirTx) ? "TX" : "RX", nextWrIdx,
            rdIdx, channel->trbChainDescBufferSz);
  }
 ...
 * @param numTRBs
 */
static void programTRBs(CUSBDMA_DmaChannel *channel,
                        uint8_t toggleFirstCycleBit,
                        const CUSBDMA_DmaTransferParam *params,
                        uint32_t startIdx, uint32_t numTRBs) {
  ...
  while ((trbCount++) < numTRBs) {
    ...
    if (trbIdx == (trbBufferSize - 1U)) {
      ...
      BUILD_LINK_TRB(trbPtr, (uint32_t)trbBufferBaseAddr, trbCycleBit,
                     TDF_TOGGLE_CYCLE_BIT);
      channel->trbCycleBit = channel->trbCycleBit ^ TDF_CYCLE_BIT;
    } else {
      /* Insert Data TRB CH = 1, TC = 0 */
      BUILD_NORMAL_TRB_NO_IOC_CHAIN(trbPtr, dmaAddr, chMaxTrbLen, sid,
                                    trbCycleBit);
      dmaAddr += chMaxTrbLen;
  ...
  if (params->sid == 0U) {
    vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
            "EP%d-%s TRB_NORMAL trbPtr(0x%08X) dmaAddr(0x%08X) trbIdx(%d) "
            "len(%d)\n",
            channel->hwUsbEppNum, (channel->isDirTx) ? "TX" : "RX",
            &(channel->trbBufferBaseAddr[trbIdx]), dmaAddr, trbIdx,
            dataRemaining);
    BUILD_NORMAL_TRB(&(channel->trbBufferBaseAddr[trbIdx]), dmaAddr,
                     dataRemaining, 0, trbCycleBit);
  } else {
    /* we do not depend on IOC;
     * for ep-in we use IOT, for ep-out we use MD_EXIT + TDL */

    BUILD_NORMAL_TRB_NO_IOC(&(channel->trbBufferBaseAddr[trbIdx]), dmaAddr,
                            dataRemaining, sid, trbCycleBit);
  }
}
...
static uint32_t channelProgramDSING(CUSBDMA_DmaChannel *channel,
  ...
  vDbgMsg(
      DBG_DMA_ERR_MSG, DBG_FYI,
      "EP%d-%s DMAbuf(0x%08X) size(%d) trbChainDescIdx(%d) actualNumTRBs(%d)\n",
      channel->hwUsbEppNum, (channel->isDirTx) ? "TX" : "RX", params->dmaAddr,
      params->len, trbChainDescIdx, actualNumTRBs);
  /* If TRB chain descriptor is allocated then program TRB */
  if (retval == CDN_EOK) {
    programTRBs(channel, 0U, params,
                channel->trbChainDescListHead[trbChainDescIdx].start,
                actualNumTRBs);
  }
  return retval;
}
...
static uint32_t channelProgramDMULT(const CUSBDMA_DmaController *pD,
  ...
  if ((params->requestQueued != 0U) || (params->requestOverflowed != 0U)) {
    ...
    if (trbChainDescIdx >= 0) {
      trbStartIdx =
          (int32_t)channel->trbChainDescListHead[trbChainDescIdx].start;
    }
  } else {
  ...
  if (trbStartIdx < 0) {
    ...
    trbRdWrDist = ((trbBufferSz + channel->trbBufferEnqueueIdx) -
                   channel->trbBufferDequeueIdx) %
                  trbBufferSz;

    /* Update the control word only if Distance is at-least 2 TRB */
    if (trbRdWrDist >= 2U) {
      channel->trbBufferBaseAddr[trbStartIdx].ctrl =
          trbCtrlWord ^ TDF_CYCLE_BIT;
    } else {
...
static void updateTRBDescChainState(const CUSBDMA_DmaChannel *channel) {
  ...
  while (trbChainIdx != channel->trbChainDescWriteIdx) {
    CUSBDMA_DmaTrbChainDesc *trbChainDesc =
        &channel->trbChainDescListHead[trbChainIdx];
...
static void channelUpdateState(const CUSBDMA_DmaController *pD,
  ...
  uint32_t trbBufferEnd  = (uint32_t)(
      trbBufferBase + (channel->trbBufferSize * sizeof(CUSBDMA_DmaTrb)));
  ...
  if ((epTrAddr >= trbBufferBase) && (epTrAddr < trbBufferEnd)) {
    channel->trbBufferDequeueIdx =
        (epTrAddr - trbBufferBase) / (uint32_t)sizeof(CUSBDMA_DmaTrb);
  }
 ...
 * @param channel pointer to channel object
 */
static void channelTrigger(const CUSBDMA_DmaController *pD,
                           CUSBDMA_DmaChannel *channel) {
  DmaRegs *regs = pD->regs;
  ...
  if ((channel->status == CUSBDMA_STATUS_FREE) &&
    ...
    if (currentDequeueAddr == 0U) {
      vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
              "Warning: Patching EP_TRADDR:  0x%08X\n", deQueueAddr_l);
      CPS_UncachedWrite32(&regs->traddr, deQueueAddr_l);
    }

    if (current_ext_addr != deQueueAddr_h) {
      vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
              "Warning: Patching EP_EXT_ADDR old(0x%08X) new(0x%08X) \n",
    ...
    if ((channel->trbBufferDmultDelinkIdx >= 0) &&
        (channel->trbBufferDequeueIdx ==
         (uint32_t)channel->trbBufferDmultDelinkIdx)) {
    ...
    if (channel->currentStreamID == 0U) {
      ...
      uint32_t cmd = DMARF_EP_DRDY | DMARF_EP_ERDY |
                     (tdl << DMARF_EP_TDL_OFST) | DMARF_EP_STDL |
...
static uint32_t channelDisable(DmaRegs *regs) {
  ...
  while ((ep_sts & DMARF_EP_DBUSY) == DMARF_EP_DBUSY) {
    ...
    if (counter >= CUSBDMA_DEFAULT_TIMEOUT) {
      ...
      vDbgMsg(DBG_DMA_BASIC_MSG, 1,
              "Warning: Timeout waiting for DMA busy to clear %d \n", counter);
      ret = CDN_ETIMEDOUT;
      break;
    }
  }
 ...
 * @param sysReq pointer to structure where DMA driver memory requirements will
 * be stored
 * @return CDN_EOK on success, error code elsewhere
 */
...
uint32_t CUSBDMA_Init(CUSBDMA_DmaController *pD, const CUSBDMA_Config *config) {
  ...
  if (ret == CDN_EOK) {
    ...
    (void)memset((void *)ctrl, 0,
                 sizeof(CUSBDMA_DmaController)); /* XDP test apps do that */
    ...
    ctrl->regs =
        (DmaRegs *)
            config->regBase; /* get pointer to the DMA HW base registers */
    /* parasoft-end-suppress MISRA2012-RULE-11_4 */
 ...
 */
static inline void channelAllocChCfg(
    CUSBDMA_DmaTrbChainDesc *trbChainDescListHead, CUSBDMA_DmaChannel *channel,
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a "Pass parameter ctrl with const
 * specifier", DRV-4352*/

/* Note that parameter 'ctrl' is used for obtaining non-const
 * 'CUSBDMA_DmaChannel *' */
static uint32_t channelAllocChSel(CUSBDMA_DmaController *ctrl,
                                  CUSBDMA_DmaChannel **channel, uint8_t isDirTx,
                                  uint8_t hwEpNum) {

  uint32_t ret = CDN_EOK;
  uint32_t trbChainSz =
      (uint32_t)(sizeof(ctrl->trbChainDesc) / sizeof(CUSBDMA_DmaTrbChainDesc));
  ...
  if (isDirTx != 0U) {
    ...
    if (((hwEpNum + 16U) > 31U) ||
        (ctrl->tx[hwEpNum].status >= CUSBDMA_STATUS_FREE)) {
      ret = CDN_EINVAL;
    } else {
      channelNum += 16U;
  ...
  if (ret == CDN_EOK) {
    channelAllocChCfg(&ctrl->trbChainDesc[channelNum * epTrbDescChainSz],
                      *channel, &(*ctrl->epMemRes)[channelNum],
                      epTrbDescChainSz);
  }
  return ret;
}
 ...
 */
static void channelAllocChConf(CUSBDMA_DmaController *ctrl,
                               CUSBDMA_DmaChannel *channel,
  ...
  if (channelParams->hwEpNum < CUSBDMA_MAX_DMA_CHANNELS) {
    ...
    /* set other fields */
    vDbgMsg(
        DBG_DMA_CHANNEL_USEGE_MSG, 1,
        "Channel 0x%p has been allocated for EP%d-%s trbBufferbase_ext 0x%x\n",
        channel, channelParams->hwEpNum, (channel->isDirTx != 0U) ? "TX" : "RX",
        trbBufferbase_ext);
    ...
    CPS_UncachedWrite32(&regs->ep_sel,
                        (uint32_t)channelParams->hwEpNum | channel->isDirTx);
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelAlloc(CUSBDMA_DmaController *pD,
                              CUSBDMA_DmaChannel **channelPtr,
                              CUSBDMA_ChannelParams *channelParams) {

  uint32_t ret =
      ((pD == NULL) || (channelPtr == NULL) || (channelParams == NULL))
          ? CDN_EINVAL
          : CDN_EOK;
  ...
  if (ret == CDN_EOK) {
    ...
    ret  = channelAllocChSel(ctrl, &channel, channelParams->isDirTx,
                            channelParams->hwEpNum);
  }
  if (ret == CDN_EOK) {
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelFreeHeadTrbChain(const CUSBDMA_DmaController *pD,
                                         CUSBDMA_DmaChannel *channel) {
  ...
  if (ret == CDN_EOK) {
    ...
    if (channel->trbChainDescReadIdx != channel->trbChainDescWriteIdx) {
      ...
      vDbgMsg(
          DBG_DMA_VERBOSE_MSG, 1,
          "EP%d-%s Free ChainDescIdx(%d) TRBs start(%d) end(%d) allocated\n",
          channel->hwUsbEppNum, (channel->isDirTx != 0U) ? "TX" : "RX",
          channel->trbChainDescReadIdx, trbChainDesc->start, trbChainDesc->end);

      channel->trbChainDescReadIdx =
          (channel->trbChainDescReadIdx + 1U) % channel->trbChainDescBufferSz;
      ...
      vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
              "EP%d-%s No queued chain desc RdIdx(%d) WrIdx(%d)",
              channel->hwUsbEppNum, (channel->isDirTx != 0U) ? "TX" : "RX",
...
uint32_t CUSBDMA_ChannelReset(CUSBDMA_DmaController *pD,
  ...
  if (ret == CDN_EOK) {
    ...
    CPS_UncachedWrite32(&regs->ep_sel, ((uint32_t)channel->isDirTx |
                                        (uint32_t)channel->hwUsbEppNum));
    ...
    if ((channel->status == CUSBDMA_STATUS_FREE) &&
      ...
      vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI, "EP%d-%s Reset Pointers \r\n",
              channel->hwUsbEppNum, ((channel->isDirTx) == 0U) ? "RX" : "TX");
      ...
      vDbgMsg(DBG_DMA_ERR_MSG, DBG_FYI,
              "EP%d-%s Reset Pointers Denied: ChannelStatus(%d)\r\n",
              channel->hwUsbEppNum, ((channel->isDirTx) == 0U) ? "RX" : "TX",
              channel->status);
      ret = CDN_EPERM;
    }
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelRelease(CUSBDMA_DmaController *pD,
                                CUSBDMA_DmaChannel *channel) {
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelProgram(CUSBDMA_DmaController *pD,
                                CUSBDMA_DmaChannel *channel,
                                const CUSBDMA_DmaTransferParam *params) {
  ...
  if (retval == CDN_EOK) {
    ...
    vDbgMsg(DBG_DMA_BASIC_MSG, 1,
            "EP%d-%s DMA prepare for channel: 0x%p  dmaAddr 0x%lx length %d\n",
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelTrigger(CUSBDMA_DmaController *pD,
                                CUSBDMA_DmaChannel *channel) {
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelUpdateState(CUSBDMA_DmaController *pD,
                                    CUSBDMA_DmaChannel *channel) {
 ...
 * @return CDN_EOK on success, error code elsewhere
 */
uint32_t CUSBDMA_ChannelSetMaxPktSz(CUSBDMA_DmaController *pD,
                                    CUSBDMA_DmaChannel *channel,
  ...
  if (ret == CDN_EOK) {
    ...
    if (value != 0U) {
      ...
      if ((channel->status == CUSBDMA_STATUS_FREE) &&
        ...
        /* set stall */
        CPS_UncachedWrite32(&regs->ep_cmd, DMA_EP_CMD_SSTALL | DMA_EP_CMD_ERDY |
                                               DMA_EP_CMD_DFLUSH);
      ...
      if (ret == CDN_EOK) {
        uint32_t dequeuePtr = (uint32_t)getPhyAddrOfTRBPtr(
            channel, &channel->trbBufferBaseAddr[channel->trbBufferDequeueIdx]);
        channel->trbBufferDequeueIdx = channel->trbBufferEnqueueIdx;
