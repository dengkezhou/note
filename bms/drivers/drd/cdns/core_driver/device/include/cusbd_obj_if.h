...
typedef struct CUSBD_OBJ_s {
   ...
   * @return CDN_ENOTSUP if hardware has an inconsistent configuration or
   ...
   */
  uint32_t (*init)(CUSBD_PrivateData *pD, const CUSBD_Config *config,
   ...
   * descriptor
   */
  uint32_t (*epEnable)(CUSBD_PrivateData *pD, CUSBD_Ep *ep,
                       const uint8_t *desc);
   ...
   * @return CDN_EINVAL if pD or ep is NULL
   */
  uint32_t (*epSetHalt)(CUSBD_PrivateData *pD, const CUSBD_Ep *ep,
                        uint8_t value);
   ...
   * @return 0 on success
   * @return CDN_EPROTO only on default endpoint if endpoint is not in data
   * stage phase
   * @return CDN_EPERM if endpoint is not enabled
   * @return CDN_EINVAL if pD, ep, or req is NULL
   */
  uint32_t (*reqQueue)(CUSBD_PrivateData *pD, const CUSBD_Ep *ep,
                       CUSBD_Req *req);
   ...
   * @return CDN_EINVAL if illegal/inconsistent values in 'config'
   */
  uint32_t (*dGetConfigParams)(const CUSBD_PrivateData *pD,
