...
struct CUSBDMA_MemResources_s {
  ...
  /** Physical address extension to 48 bits, this variable is a 16 bit width
   * page index */
  uint16_t memPageIndex;
};

/** Configuration of DMA.  Object of this type is used for probe and init
 * functions when checking memory requirements of device object and for hardware
 * controller configuration.  Size of device object in bytes is returned on
 * probe function call in CUSBDMA_SysReq.privDataSize field. */
struct CUSBDMA_Config_s {
  /** Base address of DMA special function register. */
  uintptr_t regBase;
  /** Field holds information about DMA mode.  DMA mode can be programmed
   * globally for all channels or individually for particular channels.  Each
   * bit of dmaModeTx field is responsible for one DMA channel. If this field
   * will be set to 0xFFFF or 0xFFFF for both dmaModeTx and dmaModeRx then DMA
   * mode will be programmed globally, elsewhere each DMA channel will be
   * programmed individually in channelAlloc function. Meanings of bits: 0 DMA
   * Single Mode, 1 DMA DMULT Mode. */
  uint16_t dmaModeTx;
  /** Field holds information about DMA mode.  DMA mode can be programmed
   * globally for all channels or individually for particular channels.  Each
   * bit of dmaModeTx field is responsible for one DMA channel. If this field
...
struct CUSBDMA_SysReq_s {
  ...
  /** Size of memory required for USB Transfer Request Descriptors.  This memory
   * will be used by DMA controller, so it should be suitable for DMA operation.
   */
  uint32_t trbMemSize;
};
