...
/** Number of channel supported by DMA driver. This is used internally by the
 * driver. */
#define CUSBDMA_MAX_DMA_CHANNELS 16U

/** Number of element for holding information about TRB chain. We assume that
 * for every channel for IN and OUT direction we can allocate average one TRBS
 * chain. */
...
typedef enum {
  ...
  /** DMA has been armed and Doorbell or DMA BUSY bit is still set.  If those
     bits are set it indicate that DMA is armed and data has not been transfered
     yet */
 ...
 * @param[in] pD driver state info specific to this instance
 * @param[in] channel channel pointer to DMA channel for which transfer will be
 * started
