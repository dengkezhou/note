...
static uint32_t checkControllerID(USB_SspDrdRegs *regsBase) {
  ...
  if ((USB_SSP_DRD_DID_VALUE != didVal) || (USB_SSP_DRD_RID_VALUE != ridVal)) {
    DbgMsg(DBG_GEN_MSG, DBG_CRIT,
           "ERROR %s: Controller ID read from register doesn't match an ID of "
...
static uint32_t waitForIpReady(USB_SspDrdRegs *regsBase) {
  ...
  if (false == isReady) {
    ...
    status = CDN_EOK;
    DbgMsg(DBG_GEN_MSG, DBG_FYI, "%s: DRD controller is ready.\n", __func__);
  }
  return status;
}
