 ...
 * Enumerations
 **********************************************************************/
/** Current mode of controller. Regards hardware strap mode and software enabled
 * operation */
typedef enum {
 ...
 * Callbacks
 **********************************************************************/
/** This is a generic callback. It is used to controll depended host and device
 * drivers. */
...
typedef uint32_t (*USB_SSP_DRD_InterruptHandler)(
    USB_SSP_DRD_PrivData *privData);

/** Type definition for transition function for an event of ID pin level change
 * to high. */
...
/** Type definition for transition function for an event of ID pin level change
 * to high and VBUS level change to low. */
typedef uint32_t (*USB_SSP_DRD_EventIdUpVBusDown)(
    USB_SSP_DRD_PrivData *privData);

/** Type definition for transition function for an event of ID pin level change
 * to high and VBUS level change to high. */
typedef uint32_t (*USB_SSP_DRD_EventIdUpVBusUp)(USB_SSP_DRD_PrivData *privData);

/** Type definition for transition function for an event of ID pin level change
 * to low and VBUS level change to low. */
typedef uint32_t (*USB_SSP_DRD_EventIdDownVBusDown)(
    USB_SSP_DRD_PrivData *privData);

/** Type definition for transition function for an event of ID pin level change
 * to low and VBUS level change to high. */
typedef uint32_t (*USB_SSP_DRD_EventIdDownVBusUp)(
    USB_SSP_DRD_PrivData *privData);
 ...
 */
typedef uint32_t (*USB_SSP_DRD_EventDisableHost)(
    USB_SSP_DRD_PrivData *privData);

/** Type definition for transition function for an event of disabling device
 * mode. */
 ...
 * @return CDN_EOK On success
 * @return CDN_EINVAL If illegal/inconsistent values in 'config' doesn't support
 * feature(s) required by 'config' parameters.
 ...
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t USB_SSP_DRD_CheckIfReady(USB_SSP_DRD_PrivData *privData,
                                  bool *isReady);
 ...
 * @return CDN_EINVAL if input parameters are invalid
 */
uint32_t USB_SSP_DRD_CheckIrq(const USB_SSP_DRD_PrivData *privData,
