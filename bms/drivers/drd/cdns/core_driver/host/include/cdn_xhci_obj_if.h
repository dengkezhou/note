...
typedef struct USBSSP_OBJ_s {
   ...
   * $FROM USBSSP_EP_CONT_OFFSET $TO USBSSP_EP_CONT_MAX - (uint8_t)1 $
   * @param[in] buff Buffer for data to send or to receive
   * @param[in] size Size of data in bytes
   * @param[in] complete pointer to function which will be returned in callback
   * in input parameter, can be set to NULL when no extra parameter used
   ...
   * @return CDN_EOK if selected endpoint is within available endpoint range
   */
  uint32_t (*transferVectorData)(USBSSP_DriverResourcesT *res, uint8_t epIndex,
   ...
   * USBSSP_EP0_CONT_OFFSET $TO USBSSP_EP_CONT_MAX - (uint8_t)1 $
   ...
   * @param[in] res Driver resources
   * @param[in] base Physical address of USBSSP controller where is mapped in
   * system
   * @return CDN_EINVAL when driver's settings doesn't suit to native platform
   * settings
   * @return CDN_EOK if no errors
   ...
   * @return CDN_EOK if no errors
   */
  uint32_t (*issueGenericCommand)(USBSSP_DriverResourcesT *res, uint32_t dword0,
                                  uint32_t dword1, uint32_t dword2,
                                  uint32_t dword3);
   ...
   * USBSSP_EP_CONT_OFFSET $TO USBSSP_EP_CONT_MAX - (uint8_t)1 $
   * @param[in] feature When 1 sets stall, when 0 clears stall
   * @return CDN_EOK on success
   * @return complete_code XHCI transfer complete status code
   */
  uint32_t (*endpointSetFeature)(USBSSP_DriverResourcesT *res, uint8_t epIndex,
                                 uint8_t feature);
   ...
   */
  uint32_t (*setPortOverrideReg)(const USBSSP_DriverResourcesT *res,
                                 uint32_t regValue);
   ...
   */
  uint32_t (*getPortOverrideReg)(const USBSSP_DriverResourcesT *res,
                                 uint32_t *regValue);
   ...
   */
  uint32_t (*getPortConnected)(const USBSSP_DriverResourcesT *res,
                               uint8_t *connected);
   ...
   */
  uint32_t (*getDevAddressState)(const USBSSP_DriverResourcesT *res,
                                 uint8_t *addrState);
