...
#define USBSSP_XFER_DESC_QUEUE_SIZE                                            \
  (((USBSSP_PRODUCER_QUEUE_SIZE + 7U) >> 3U) + 1U)
...
/** Value returned by USBSSP_Isr to report occurrence of unexpectedly large
 * number of consecutive XHCI events. */
...
/** Used for testing purposes */
typedef void (*USBSSP_PreportChangeDetectCallback)(uint32_t portsc_value,
                                                   uint32_t port_id);
