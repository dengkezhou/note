...
USBSSP_EpContexEpState getEndpointStatus(USBSSP_DriverResourcesT const *res,
                                         uint32_t epIndex) {
  ...
  if (epIndex < USBSSP_EP0_CONT_OFFSET) {
    ...
    if (epIndex == USBSSP_EP0_CONT_OFFSET) {
      hwValue = (le32ToCpu(res->xhciMemRes->outputContext->ep0Context[0]) &
                 (uint32_t)USBSSP_EP_CONTEXT_STATE_MASK);
    } else if (epIndex <
               USBSSP_EP_CONT_MAX) { /* get status of no default endpoint */
      ...
      hwValue =
          (le32ToCpu(res->xhciMemRes->outputContext->epContext[epBase][0]) &
           (uint32_t)USBSSP_EP_CONTEXT_STATE_MASK);
    } else {
      hwValue = 0U;
    }
...
CH9_UsbSpeed getSpeed(const USBSSP_DriverResourcesT *res) {
  ...
  uint8_t portSpeed =
      (uint8_t)CPS_FLD_READ(USBSSP__PORTSC1USB3, PORTSPEED, portStatus);
...
static void setSlotContext(USBSSP_DriverResourcesT *res) {
  ...
  res->inputContext->slot[0] =
      cpuToLe32((uint32_t)(1UL << USBSSP_SLOT_CXT_CXT_ENT_POS) |
                (uint32_t)(slotSpeed << USBSSP_SLOT_CONTEXT_SPEED_POS));

  res->inputContext->slot[1] =
      cpuToLe32((uint32_t)res->actualPort << USBSSP_SLOT_CXT_PORT_NUM_POS);
  ...
  res->inputContext->ep0Context[1] = cpuToLe32(
      ...
      (USBSSP_EP_CONTEXT_3ERR
       << USBSSP_EP_CONTEXT_CERR_POS)); /* endpoint 0 control type, max packet,
                                           error count */
  ...
  res->inputContext->ep0Context[4] = cpuToLe32(((uint32_t)(
      USBSSP_EP_CXT_EP_CTL_AVGTRB_LEN
      << USBSSP_EP_CXT_EP_AVGTRBLEN_POS))); /* endpoint 0 control type - average
                                               trb length must be set to 8. */
}
...
static void setAddressCommandTrb(USBSSP_DriverResourcesT *res, uint8_t bsrVal) {
  ...
  res->commandQ.enqueuePtr->dword3 = cpuToLe32(
      ...
      ((uint32_t)bsrVal << USBSSP_BSR_POS) | (uint32_t)res->commandQ.toogleBit);
...
void setAddress(USBSSP_DriverResourcesT *res, uint8_t bsrVal) {
  ...
  if (errorAndReturn == 0U) {
    ...
    res->inputContext->inputControlContext[1] =
        cpuToLe32(3); /* A0/A1=1/1, all other Ax = 0 */
...
static void insertLinkTRB(USBSSP_ProducerQueueT *queue,
  ...
  vDbgMsg(
      ...
      "LinkTRB contextIndex:%d linkAddr:0x%16x  dword3:0x%08x\n",
      queue->contextIndex,
      (((uint64_t)queue->ring[USBSSP_PRODUCER_QUEUE_SIZE - 1U].dword1) << 32) |
...
void updateQueuePtr(USBSSP_ProducerQueueT *queue, uint32_t linkTrbChainFlag) {
  ...
  USBSSP_RingElementT *poolEnd =
      &queue->ring[(USBSSP_PRODUCER_QUEUE_SIZE - 2U)];
  ...
  vDbgMsg(
      USBSSP_DBG_DRV, DBG_FYI,
      "contextIndex:%d oldPtr:0x%08x enqueuePtr:0x%08x dequeuePtr:0x%08x \n",
...
void noOpTest(USBSSP_DriverResourcesT *res) {
  ...
  res->commandQ.enqueuePtr->dword3 =
      cpuToLe32((uint32_t)((USBSSP_TRB_NO_OP_COMMAND << USBSSP_TRB_TYPE_POS) |
...
static void nbControlTransferSetup(USBSSP_DriverResourcesT *res,
  ...
  res->ep0.enqueuePtr->dword0 =
      cpuToLe32(((uint32_t)le16ToCpu(setup->wValue) << USBSSP_TRB_WVALUE_POS) |
  ...
  res->ep0.enqueuePtr->dword2 =
      cpuToLe32((uint32_t)8U); /* setup is always 8 bytes length */
 ...
 * @param dir data direction flag
 */
static void nbControlTransferData(USBSSP_DriverResourcesT *res,
 ...
 */
void enqueueNBControlTransfer(USBSSP_DriverResourcesT *res,
                              const CH9_UsbSetup *setup, const uint8_t *pdata) {
 ...
 */
void xhciGetDescXferComplete(USBSSP_DriverResourcesT *res, uint32_t status,
 ...
 * @param eventPtr Pointer to the completed event
 */
void xhciGetShortCfgDescComplete(USBSSP_DriverResourcesT *res, uint32_t status,
                                 const USBSSP_RingElementT *eventPtr) {
 ...
 * @param eventPtr Pointer to the completed event
 */
static void epClearFeatureComplete(USBSSP_DriverResourcesT *res,
                                   uint32_t status,
                                   const USBSSP_RingElementT *eventPtr) {
  ...
  if (status == CDN_EOK) {
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
            "<%d> epIndex: %02x status(%d) completionCode(%x)\n",
 ...
 * @param feature
 */
void xhciEpSetFeatureHost(USBSSP_DriverResourcesT *res, uint8_t epIndex,
                          uint8_t feature) {
  ...
  epAddress =
      (epIndex / 2U) | (((epIndex % 2U) != 0U) ? CH9_USB_EP_DIR_IN : 0U);
