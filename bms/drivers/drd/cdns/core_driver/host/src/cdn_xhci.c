...
#define USBSSP_DEFAULT_TIMEOUT 1000U
#endif
...
/*local functions*/
static void configureEndpoints(USBSSP_DriverResourcesT *res,
                               uint8_t const *conf);
 ...
 */
static uint32_t waitForVal(volatile uint8_t const *value, uint32_t waitFor,
                           uint32_t timeout) {
 ...
 */
static uint32_t waitUntilEpStoppedDisabled(USBSSP_DriverResourcesT const *res,
                                           uint32_t timeout) {
...
static uint32_t checkEpXferEnabled(USBSSP_DriverResourcesT const *res,
                                   uint8_t epIndex) {
  uint32_t ret = CDN_EOK;
...
static void getDescriptorHubReq(uint8_t descType, uint8_t *setup) {
  ...
  if (descType == CH9_USB_DT_USB2_HUB) {
    setup[0] = (uint8_t)(CH9_USB_DIR_DEVICE_TO_HOST | CH9_USB_REQ_TYPE_CLASS |
    ...
    setup[0] =
        (uint8_t)(CH9_USB_DIR_DEVICE_TO_HOST | CH9_USB_REQ_TYPE_STANDARD |
                  CH9_USB_REQ_RECIPIENT_DEVICE);
  }
}
#endif
...
uint32_t USBSSP_GetDescriptor(USBSSP_DriverResourcesT *res, uint8_t descType,
  ...
  if (ret == CDN_EOK) {
    ...
    uint8_t setup[] = { 
        0x80U, 
        (uint8_t)CH9_USB_REQ_GET_DESCRIPTOR,
        0x00U, 
        descType,
        0x00U, 
        0x00U,
    ...
    switch( descType ){
        ...
        break;
        case CH9_USB_DT_INTERFACE:
            //do sth
        break;
        case CH9_USB_DT_ENDPOINT:
            //do sth
        break;
        default:
        return CDN_EOK;
 ...
 */
static uint32_t setConfigurationSetupReq(USBSSP_DriverResourcesT *res,
                                         uint32_t configValue) {
...
uint32_t USBSSP_SetConfiguration(USBSSP_DriverResourcesT *res,
  ...
  if (ret == (uint32_t)CDN_EOK) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
            "<%d> SET CONFIGURATION setup completed.\n", res->instanceNo);
...
uint8_t USBSSP_CalcFsLsEPIntrptInterval(uint8_t bInterval) {
  ...
  uint8_t res2 =
      0U; /* used for finding the first 1 looking from right to left */
 ...
 */
static void initStreamObj(USBSSP_ProducerQueueT const *epObj,
                          USBSSP_ProducerQueueT *stream, uint32_t iter) {

  USBSSP_DriverResourcesT *res = epObj->parent;

  /* initialize ring of stream */
  stream->ring       = (USBSSP_RingElementT *)(&(
      *res->xhciMemRes->streamRing)[epObj->contextIndex - 2U][iter][0]);
  stream->enqueuePtr = stream->ring;
  stream->dequeuePtr = stream->ring;
  stream->contextIndex =
      epObj->contextIndex; /* All child stream objects have parent's (endpoint)
                              contextIndex */
  stream->toogleBit      = 1U;
...
static void setMaxPStreams(USBSSP_ProducerQueueT *epObj) {
  ...
  if (epDescType == CH9_USB_EP_BULK) {
    uint8_t hwMaxPStreams = (uint8_t)CPS_FLD_READ(
        USBSSP__HCCPARAMS, MAXPSASIZE, res->qaRegs.xHCCaps.hccparams1);
    uint32_t streamId;
    ...
    if (maxPStreams > 0U) {
      ...
      if (((int16_t)epContext < (int16_t)USBSSP_MAX_EP_NUM_STRM_EN) &&
          ((int16_t)epContext >= 0)) {
        for (streamId = 0; streamId < (USBSSP_STREAM_ARRAY_SIZE - 1U);
             streamId++) {

          USBSSP_ProducerQueueT *stream;
          USBSSP_ProducerQueueT(
              *streamObj)[USBSSP_MAX_EP_NUM_STRM_EN][USBSSP_STREAM_ARRAY_SIZE] =
...
static void setInterval(USBSSP_ProducerQueueT *epObj) {
  ...
  /* Convert interval to endpoint valid value (Table 65, section 6.2.3.6 of the
   * xHCI Spec). */
  ...
  case CH9_USB_SPEED_FULL:
    /* Table 65 - FS Isoch. - convert interval (1-16) to (3-18) i.e. increment
     * by 2. */
  ...
  default:
    /* Table 65 - SS or HS (Interrupt/Isoch) - convert interval (1-16) to (0-15)
     * i.e. decrement by 1. */
...
static void setMaxESITPayload(USBSSP_ProducerQueueT *epObj) {
  ...
  uint16_t epMaxPacketSize =
      (((uint16_t)desc[5] & 0x7U) << 8) | (uint16_t)desc[4];
  ...
  case CH9_USB_SPEED_HIGH:
    bytesPerInterval =
        handleSpeedHigh(epDescType, epMaxPacketSize, burst_value);
    break;
  ...
  if (bytesPerInterval > (48U * 1024U)) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_HIVERB,
            "<%d> WARNING:  bytesPerInterval(%d) > 48KB, Patching to 48KB\n",
  ...
  if (bytesPerInterval > 0U) {
    epObj->hwContext[0] |=
        ((bytesPerInterval >> 16U) << USBSSP_EP_CXT_MAXESITPLD_HI_POS);
    epObj->hwContext[4] |=
        ((bytesPerInterval & 0xFFFFU) << USBSSP_EP_CXT_MAXESITPLD_LO_POS);
  }
}
...
static void setMaxBurstSize(USBSSP_ProducerQueueT *epObj) {
  ...
  if (maxBurstSize > 0U) {
    epObj->hwContext[1] |=
        ((uint32_t)maxBurstSize << USBSSP_EP_CXT_MAX_BURST_SZ_POS) &
 ...
 */
static uint32_t storeEpDesc(USBSSP_DriverResourcesT const *res,
                            USBSSP_ProducerQueueT *epObj, uint8_t const *desc) {
  ...
  if (res->actualSpeed >= CH9_USB_SPEED_SUPER) {
    ...
    if (desc[CH9_USB_DS_ENDPOINT] != CH9_USB_DS_SS_USB_EP_COMPANION) {
      ...
      vDbgMsg(
          ...
          "<%d> Endpoint companion descriptor does not exist for SSx device\n",
          res->instanceNo);
    } else {
      (void)memcpy(&epObj->epDesc[CH9_USB_DS_ENDPOINT],
                   &desc[CH9_USB_DS_ENDPOINT], CH9_USB_DS_SS_USB_EP_COMPANION);
    }
  }
  return ret;
}
 ...
 */
static uint32_t updateEpInputContext(USBSSP_DriverResourcesT *res,
  ...
  if (epObj->contextIndex < 32U) {
    ...
    /* If endpoint was previously registered to be dropped (Dx==1)_clearing Dx
     * flag */
    ...
    res->inputContext->slot[0] = cpuToLe32(
        ...
        ((uint32_t)getSlotSpeed(res->actualSpeed)
         << USBSSP_SLOT_CONTEXT_SPEED_POS)); /*6.2.2 set slot context entries
                                                and speed */
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
            "<%d> Illegal value of context number %d\n", res->instanceNo,
            epObj->contextIndex);
    ret = CDN_EINVAL;
  }
  return ret;
...
uint32_t USBSSP_EnableEndpoint(USBSSP_DriverResourcesT *res,
  ...
  if (ret != CDN_EOK) {
    ...
    epIndex =
        (uint8_t)((((epAddress & 0x7FU) - 1U) * 2U) + ((epIn > 0U) ? 1U : 0U));
...
static void configureEndpoints(USBSSP_DriverResourcesT *res,
  ...
  res->inputContext->inputControlContext[1] =
      cpuToLe32(1); /* A0 = 1, all other Ax = 0 */
  if (res->inputContextCallback != NULL) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI, "<%d> Calling inputContextCallback()\n",
            res->instanceNo);
    res->inputContextCallback(res);
...
uint32_t USBSSP_AddEventDataTRB(USBSSP_DriverResourcesT *res, uint8_t epIndex,
  ...
  if (ret == CDN_EOK) {
    ...
    USBSSP_ProducerQueueT *ep =
        (epIndex == USBSSP_EP0_CONTEXT_OFFSET) ? &res->ep0 : &res->ep[epIndex];
    ...
    ep->enqueuePtr->dword3 =
        cpuToLe32((uint32_t)((USBSSP_TRB_EVENT_DATA << USBSSP_TRB_TYPE_POS) |
                             flags | (uint32_t)ep->toogleBit));
...
static uint16_t getMaxPacketSize(const USBSSP_ProducerQueueT *epObj) {
  ...
  uint16_t maxPacketSize =
      (uint16_t)(epObj->hwContext[1] >> USBSSP_EP_CXT_MAX_PKT_SZ_POS);
...
static uint32_t calculateTdSize(uint32_t singleTrbLength,
  ...
  if (tdParams->epMaxPacketSize > 0U) {
    ...
    tdParams->packetTransfered =
        tdParams->trbTransferLengthSum / tdParams->epMaxPacketSize;
  }
...
static uint32_t getBurstCount(const USBSSP_ProducerQueueT *ep,
  ...
  uint32_t burstCount =
      (tdParams->tdPacketCount + burstSize + 1U) / (burstSize + 1U);
 ...
 * @param tdParams pointer to extra parameters (used internally)
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Parasoft false violation
 * with const specifier for res, DRV-3821" */
...
static void createSingleTrb(USBSSP_DriverResourcesT *res, uintptr_t dataPtr,
  ...
  ep->enqueuePtr->dword2 =
      cpuToLe32((((uint32_t)ep->interrupterIdx) << USBSSP_TRB_INTR_TRGT_POS) |
                (singleTrbLength & 0x0001FFFFU) | (tdSize << 17));

  ep->enqueuePtr->dword3 = cpuToLe32((uint32_t)(
      (trbType << USBSSP_TRB_TYPE_POS) | flags | (uint32_t)ep->toogleBit));
  ...
  vDbgMsg(
      USBSSP_DBG_DRV, DBG_FYI,
      "ContextIndex:%d NOMAL_TRB dw0:0x%08x dw1:0x%08x dw2:0x%08x dw3:0x%08x\n",
      ep->contextIndex, ep->enqueuePtr->dword0, ep->enqueuePtr->dword1,
      ep->enqueuePtr->dword2, ep->enqueuePtr->dword3);
  ...
  vDbgMsg(
      ...
      "ContextIndex:%d address: 0x%x, length: %d, tdSize: %d, flags: %08x, "
      "trbTransferLengthSum: %d, packetTransfered: %d, interrupterIdx(%d)\n",
      ep->contextIndex, dataPtr, singleTrbLength, tdSize, flags,
      tdParams->trbTransferLengthSum, tdParams->packetTransfered,
      ep->interrupterIdx);
  updateQueuePtr(ep, linkTrbChainFlag);
}
/* parasoft-end-suppress MISRA2012-RULE-8_13_a-4 "Parasoft false violation with
 * const specifier for res, DRV-3821" */
 ...
 */
static uint32_t calculateSingleTrbLength(uint32_t numOfTrb, uint32_t dataLength,
                                         uintptr_t buffStart) {
  ...
  if (numOfTrb == 1U) {
    ...
    singleTrbLength = (uint32_t)(USBSSP_TRB_MAX_TRANSFER_LENGTH -
                                 (buffStart % USBSSP_TRB_MAX_TRANSFER_LENGTH));
  }
...
static void trbSinglePage(USBSSP_DriverResourcesT *res, uintptr_t *dataPtr,
  ...
  for (trbIndex = 0U; trbIndex < numOfTrb; trbIndex++) {
    ...
    if (trbIndex == 0U) {
      ...
      singleTrbLength =
          calculateSingleTrbLength(numOfTrb, dataLength, buffStart);
...
static uint32_t calcLastPageNumBytes(uintptr_t buff, uint32_t size) {
  uint32_t numOfBytes =
      (uint32_t)((buff + (uintptr_t)size) % USBSSP_SYSTEM_MEMORY_PAGE_SIZE);
  return numOfBytes;
}
...
static void createTdAllMemPagesIndex(USBSSP_TDCreateT *tdParams,
  ...
  if (size > 0U) {
    tdParams->pageEnd =
        ((buff + (uintptr_t)size) - 1U) / USBSSP_SYSTEM_MEMORY_PAGE_SIZE;
  } else {
...
static void createTdAllMemPages(USBSSP_DriverResourcesT *res,
                                USBSSP_TDCreateT *tdParams,
                                const uintptr_t buff, uint32_t const size) {
  ...
  vDbgMsg(
      USBSSP_DBG_DRV, DBG_HIVERB,
      "<%d> pageStart: 0x%x, pageEnd: 0x%x, numOfpages: %d tdPacketCount: %d\n",
  ...
  for (pageIndex = 0; pageIndex < tdParams->numOfPages; pageIndex++) {
    ...
    if (pageIndex == 0U) {
      ...
      numOfBytes = calcFirstPageNumBytes(tdParams->pageStart, tdParams->pageEnd,
                                         buff, size);
      /* for last page */
 ...
 */
static void USBSSP_CreateTD(USBSSP_DriverResourcesT *res, uint8_t const index,
                            const uintptr_t buff, uint32_t const size,
...
static uint32_t USBSSP_TransferDataDRBL(USBSSP_DriverResourcesT *res,
                                        USBSSP_ProducerQueueT const *ep) {
  ...
  if (endpointState != USBSSP_EP_CONTEXT_EP_STATE_HALTED) {
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
            "<%d> ENDPOINT %d is in not RUNNING state, can not issue DOORBELL "
            "- current status: %d\n",
 ...
 */
static uint32_t calcSGDataLength(const USBSSP_XferBufferDesc *bufferDesc,
 ...
 * @return CDN_EOK if success, error code elsewhere
 */
uint32_t USBSSP_TransferVectorData(USBSSP_DriverResourcesT *res,
                                   uint8_t epIndex,
  ...
  if (ret == CDN_EOK) {
    ...
    for (i = 0; i < (bufferCount - 1U); i++) {
      USBSSP_CreateTD(res, epIndex, bufferDesc[i].buffVec,
                      bufferDesc[i].sizeVec, &tdParams);
    }

    /* put last TRB with IOC enabled: clear all extra flags */
    ep->extraFlags        = ~((uint8_t)USBSSP_EXTRAFLAGSENUMT_FORCELINKTRB);
    tdParams.isLastBuffer = 1U;
    USBSSP_CreateTD(res, epIndex, bufferDesc[i].buffVec, bufferDesc[i].sizeVec,
                    &tdParams);
    USBSSP_WriteDoorbell(res, res->actualdeviceSlot, drblReg);
    ep->isRunningFlag = 1;
    /* start DRBL */
...
uint32_t USBSSP_ResetRootHubPort(USBSSP_DriverResourcesT const *res) {
  ...
  if (ret != CDN_EOK) {
    ...
    uint32_t reg =
        xhciRead32(&res->regs.xhciPortControl[portIndex - 1U].portsc);
    ...
    do {
      ...
      pls = (reg & USBSSP__PORTSC1USB3__PLS_MASK) >>
            USBSSP__PORTSC1USB3__PLS_SHIFT;
...
static void setSwRegs(USBSSP_DriverResourcesT *res, uintptr_t base) {
  ...
  xHCCaps->caplength_hciver =
      xhciRead32(&regs->xhciCapability->caplength_hciver);
  printf("caplength_hciver reg  : 0x%x\n", &regs->xhciCapability->caplength_hciver);
  printf("caplength_hciver value: 0x%x\n", xHCCaps->caplength_hciver);
...
static void initSfrObjDispInterrupter(USBSSP_DriverResourcesT *res) {
   ...
   */
  numOfInterrupters =
      (uint8_t)CPS_FLD_READ(USBSSP__HCSPARAMS1, MAXINTRS, xHCCaps->hcsparams1);
...
static void initSfrObjDispPorts(USBSSP_DriverResourcesT *res) {
  ...
  numOfPorts =
      (uint8_t)CPS_FLD_READ(USBSSP__HCSPARAMS1, MAXPORTS, xHCCaps->hcsparams1);
...
static void initSfrObjDispDevSlotNum(USBSSP_DriverResourcesT *res) {
   ...
   */
  maxDeviceSlot =
      CPS_FLD_READ(USBSSP__HCSPARAMS1, MAXSLOTS, xHCCaps->hcsparams1);
  /* if override  condition */
  if (res->maxDeviceSlot != 0U) {
    maxDeviceSlot = (res->maxDeviceSlot > maxDeviceSlot) ? maxDeviceSlot
                                                         : res->maxDeviceSlot;
  }
  ...
  if (maxDeviceSlot > USBSSP_MAX_DEVICE_SLOT_NUM) {
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
            "<%d> Limiting Number of Slots from(%d) to(%d)\n", res->instanceNo,
            maxDeviceSlot, USBSSP_MAX_DEVICE_SLOT_NUM);
  } else {
...
static void initSfrObjDispExtCap(USBSSP_DriverResourcesT *res) {
  ...
  vDbgMsg(
      USBSSP_DBG_DRV, DBG_FYI,
      "<%d> ExtCap base address: 0x%p, USBLEGSUP=0x%08x / USBLEGCTLS=0x%08x\n",
  ...
  for (i = 0U; i < regs->xhciExtCaps.extCapsCount; i++) {
    vDbgMsg(
        USBSSP_DBG_DRV, DBG_FYI,
        "<%d> Ext. cap. %d @(%p): firstCapPtr:%p, DWORD[0]=0x%08x, capID=%d\n",
...
static void initSfrObjExtCap(USBSSP_DriverResourcesT *res) {
  ...
  nextCapPtr     = (uint8_t)CPS_FLD_READ(USBSSP__XECP_USBLEGSUP, NEXTCP,
                                     regs->xhciExtCaps.usbLegSup);
  ...
  while ((numOfExtCaps < USBSSP_MAX_EXT_CAPS_COUNT) && (nextCapPtr > 0U)) {
    ...
    nextCapPtr =
        (uint8_t)CPS_FLD_READ(USBSSP__XECP_USBLEGSUP, NEXTCP, firstExtCapDword);
    /* Capability ID is always in the same place */
    capId =
        (uint8_t)CPS_FLD_READ(USBSSP__XECP_USBLEGSUP, CID, firstExtCapDword);
...
static uint32_t initXhcSetupScratchPad(USBSSP_DriverResourcesT *res) {
  ...
  uint16_t maxScratchBuf =
      (uint16_t)((uint16_t)CPS_FLD_READ(USBSSP__HCSPARAMS2, MAXSPBUFHI,
                                        xHCCaps->hcsparams2)
                 << 5U) |
      (uint16_t)CPS_FLD_READ(USBSSP__HCSPARAMS2, MAXSPBUFLO,
                             xHCCaps->hcsparams2);
  vDbgMsg(USBSSP_DBG_DRV, DBG_HIVERB,
  ...
  if (maxScratchBuf > USBSSP_SCRATCHPAD_BUFF_NUM) {
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
            "<%d> Reducing number of scratch pad to: %d\n", res->instanceNo,
            maxScratchBuf);
  }
  ...
  for (i = 0; i < maxScratchBuf; i++) {
    /* parasoft-begin-suppress MISRA2012-RULE-11_4-4 "uint8_t* converted to
     * uint64_t*, DRV-4292" */
    res->xhciMemRes->scratchpad[i] = cpuToLe64(
        (uintptr_t)&res->xhciMemRes->scratchpadPool[i * USBSSP_PAGE_SIZE]);
    /* parasoft-end-suppress MISRA2012-RULE-11_4-4 */
  }
...
static void initXhcSetupSlotNum(USBSSP_DriverResourcesT *res) {
  ...
  xhciWrite32(&res->regs.xhciOperational->config, devSlotNum);
}
...
static uint32_t initXhcSetupDcbaa(USBSSP_DriverResourcesT *res) {
  ...
  /* parasoft-begin-suppress MISRA2012-RULE-11_4-4 "uint64_t* converted to
   * uint64_t, DRV-4293" */
  res->xhciMemRes->dcbaa->scratchPadPtr =
      cpuToLe64((uintptr_t)res->xhciMemRes->scratchpad);
  /* parasoft-end-suppress MISRA2012-RULE-11_4-4 */
  res->xhciMemRes->dcbaa->deviceSlot[0] =
      cpuToLe64((uintptr_t)res->xhciMemRes->outputContext->slot);
  /* parasoft-begin-suppress MISRA2012-RULE-11_4-4 "USBSSP_InputContexT*
   * converted to uintptr_t, DRV-4294" */
  ret = checkStructAlign("INPUT CONTEXT", (uintptr_t)res->inputContext,
                         sizeof(USBSSP_InputContexT), USBSSP_CONTEXT_ALIGNMENT,
                         USBSSP_PAGE_SIZE);
  /* parasoft-end-suppress MISRA2012-RULE-11_4-4 */

  if (ret == CDN_EOK) {
    /* parasoft-begin-suppress MISRA2012-RULE-11_4-4 "USBSSP_OutputContexT*
     * converted to uintptr_t, DRV-4295" */
    ret = checkStructAlign("OUTPUT CONTEXT",
                           (uintptr_t)res->xhciMemRes->outputContext,
                           sizeof(USBSSP_OutputContexT),
                           USBSSP_CONTEXT_ALIGNMENT, USBSSP_PAGE_SIZE);
    /* parasoft-end-suppress MISRA2012-RULE-11_4-4 */
  }
  ...
  if (ret == CDN_EOK) {
     ...
     * uint64_t, DRV-4296" */
    dcbaap_sfr_val =
        (uint64_t)(uintptr_t)(&res->xhciMemRes->dcbaa->scratchPadPtr);
    CPS_MemoryBarrier();
    /* parasoft-end-suppress MISRA2012-RULE-11_4-4 */

    xhciWrite64(&res->regs.xhciOperational->dcbaap, dcbaap_sfr_val);

    ret = checkStructAlign("DCBAA", (uintptr_t)(dcbaap_sfr_val),
                           sizeof(USBSSP_DcbaaT), USBSSP_DCBAA_ALIGNMENT,
                           USBSSP_PAGE_SIZE);
  }
  return ret;
}
...
static uint32_t initXhcSetup(USBSSP_DriverResourcesT *res) {
  ...
  uint32_t state = 0;
  uint32_t cmd = 0;
  state = xhciRead32(&xhciOperational->usbsts) & USBSSP__USBSTS__HCH_MASK;
  if (!state) {
    cmd = xhciRead32(&xhciOperational->usbcmd);
    cmd &= ~USBSSP__USBCMD__R_S_MASK;
    xhciWrite32(&xhciOperational->usbcmd, cmd);
  }

  waitForReg(&xhciOperational->usbsts, USBSSP__USBCMD__R_S_MASK, 0U,
                     USBSSP_DEFAULT_TIMEOUT);
                     */
  /*----- halt end ----------------- */
...
#if 1 
    printf("wait until the Controller Not Ready flag (CNR) in the USBSTS is 0 ");
    ret = waitForReg(&xhciOperational->usbsts,
                     USBSSP__USBSTS__CNR_MASK, 0U, USBSSP_DEFAULT_TIMEOUT);

#if CONFIG_COMPILE_RTL
    writel(0x18030c50,  ITCS_C_USB0_BASE + 0x1812C);
    writel(0xe339060e,  ITCS_C_USB0_BASE + 0x18194);
    writel(0x311802a1,  ITCS_C_USB0_BASE + 0x18130);

    writel(0x18030c50,  ITCS_C_USB1_BASE + 0x1812C);
    writel(0xe339060e,  ITCS_C_USB1_BASE + 0x18194);
    writel(0x311802a1,  ITCS_C_USB1_BASE + 0x18130);

    writel(0x18030c50,  ITCS_C_USB2_BASE + 0x1812C);
    writel(0xe339060e,  ITCS_C_USB2_BASE + 0x18194);
    writel(0x311802a1,  ITCS_C_USB2_BASE + 0x18130);

    writel(0x18030c50,  ITCS_C_USB3_BASE + 0x1812C);
    writel(0xe339060e,  ITCS_C_USB3_BASE + 0x18194);
    writel(0x311802a1,  ITCS_C_USB3_BASE + 0x18130);
#endif
...
static void initRingsInterruptsInterupters(USBSSP_DriverResourcesT *res) {
  ...
  for (interrupter = 0U; interrupter < USBSSP_MAX_NUM_INTERRUPTERS;
       interrupter++) {
    ...
    /* parasoft-begin-suppress MISRA2012-RULE-11_4-4 "uint64_t* converted to
     * uint64_t, DRV-4297" */
    erstba_sfr_val = (uint64_t)(uintptr_t)(
        &res->xhciMemRes
             ->eventRingSegmentEntry[USBSSP_ERST_ENTRY_SIZE * interrupter]);
    /* parasoft-end-suppress MISRA2012-RULE-11_4-4 */

    CPS_MemoryBarrier();

    /* Program the Interrupter Event Ring Segment Table Size (ERSTSZ) */
    xhciWrite32(&res->regs.xhciInterrupter[interrupter].erstsz,
                1); /* one event ring */
    ...
    xhciWrite32(&res->regs.xhciInterrupter[interrupter].iman,
                USBSSP__IMAN0__IE_MASK);
  }
}
...
static void initRingsInterruptsEpRings(USBSSP_DriverResourcesT *res) {
  ...
  for (i = 1U; i < (USBSSP_MAX_EP_CONTEXT_NUM + USBSSP_EP_CONT_OFFSET); i++) {
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_HIVERB,
            "<%d> Endpoint context[%d] transfer ring address: %p\n",
...
static uint32_t initRingsInterrupts(USBSSP_DriverResourcesT *res) {
  ...
  if (ret == CDN_EOK) {
    ...
    for (interrupter = 0U; interrupter < USBSSP_MAX_NUM_INTERRUPTERS;
         interrupter++) {
      /* Allocate the Event Ring Segment Table (ERST) (section 6.5). Initialize
       * ERST table entries */
      /* to point to and to define the size (in TRBs) of the respective Event
       * Ring Segment. */
      res->eventToogleBit[interrupter] = 1;
      res->xhciMemRes
          ->eventRingSegmentEntry[interrupter * USBSSP_ERST_ENTRY_SIZE] =
          cpuToLe64(
              get64PhyAddrOf32ptr(&res->eventPtrBuffer[interrupter]->dword0));
      res->xhciMemRes
          ->eventRingSegmentEntry[(interrupter * USBSSP_ERST_ENTRY_SIZE) + 1U] =
          cpuToLe64(USBSSP_EVENT_QUEUE_SIZE);
    }
...
static void resetCmdRing(USBSSP_DriverResourcesT *res) {
  ...
  res->commandQ.toogleBit = 1;
  res->commandQ.dequeuePtr =
      res->commandQ.ring; /* set dequeue pointer to the begin of ring */
  res->commandQ.enqueuePtr =
      res->commandQ.ring; /* set enqueue pointer to the begin of ring */
...
static void initEnableXhc(USBSSP_DriverResourcesT *res) {
  ...
  /* Enable system bus interrupt generation by writing a '1' to the Interrupter
   * Enable (INTE) */
...
static uint32_t initCheckDidRidReg(const USBSSP_DriverResourcesT *res) {
  ...
  if ((res->ipIdVerRegs.didRegPtr == (uintptr_t)0) ||
     ...
     * 'uint32_t*', DRV-4299" */
    uint32_t did = xhciRead32((
        uint32_t *)(res->ipIdVerRegs
                        .didRegPtr)); /* didRegPtr contains address of DID Reg*/
    /* parasoft-end-suppress MISRA2012-RULE-11_4 */
    /* parasoft-begin-suppress MISRA2012-RULE-11_4 " unsigned long converted to
     * 'uint32_t*', DRV-4320" */
    uint32_t rid = xhciRead32((
        uint32_t *)(res->ipIdVerRegs
                        .ridRegPtr)); /* ridRegPtr contains address of RID Reg*/
    /* parasoft-end-suppress MISRA2012-RULE-11_4 */
    if ((did != USBSSP_DID_VAL) || (rid != USBSSP_RID_VAL)) {
...
uint32_t USBSSP_Init(USBSSP_DriverResourcesT *res, uintptr_t base) {
  ...
  if (ret == CDN_EOK) {
    ...
    if(ret){
        printf("initCheckDidRidReg failed\n");
    }
  }
  ...
  if (ret == CDN_EOK) {
    ...
    if (ret == CDN_EOK) {
      ...
      if(ret){
        printf("initRingsInterrupts failed\n");
      }
    }
...
static void saveXHCState(const USBSSP_DriverResourcesT *res,
   ...
   * transition to 0 */
  /* we might not be able to catch the transition. Hence will poll the flag
   * after a delay. */
  CPS_DelayNs(1000);
}
...
uint32_t USBSSP_SaveState(USBSSP_DriverResourcesT *res,
  ...
  if (CDN_EOK != ret) {
    ...
    if (ret == CDN_EOK) {
      ...
      stopXhc(res);
      /* Step-3b: Wait for HCHalted bit to be set in USBSTS (USB Status
       * register) */
    ...
    if (ret == CDN_EOK) {
      ...
      /* Step-4: Read the operational and run-time registers and save it in the
       * state struct */
      /* Step-5: Set the CCS (Controller Save State) flag in USBCMD reg */
      saveXHCState(res, drvContext);

      ret = waitForReg(&xhciOperationalRegs->usbsts, USBSSP__USBSTS__SSS_MASK,
                       0U, USBSSP_DEFAULT_TIMEOUT);
    }
...
uint32_t USBSSP_RestoreState(USBSSP_DriverResourcesT *res,
                             USBSSP_DriverContextT const *drvContext) {
  ...
  if (CDN_EOK != ret) {
    ...
    /* Step-2: Restore the saved memory image of the DCBAA, Contexts and other
     * data structures */
    ...
    CPS_DelayNs(1000);
    /* Step-5b: Wait for Restore State Status (RSS) in USBSTS to transition to
     * zero. */
    ret = waitForReg(&xhciOperationalRegs->usbsts, USBSSP__USBSTS__RSS_MASK, 0U,
                     USBSSP_DEFAULT_TIMEOUT);
    if (ret != CDN_EOK) {
      vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
              "Critical error: Timed-out waiting for Restore State Status "
              "(RSS) to be (%d) \n",
              0);
    }
    ...
    if (ret == CDN_EOK) {
      uint64_t crcr_sfr_val =
          get64PhyAddrOf32ptr(&res->commandQ.ring[0].dword0);
      /* Set the Command Ring Dequeue Pointer by programming the Command Ring
       * Control Register */
...
uint32_t USBSSP_SetPortOverrideReg(const USBSSP_DriverResourcesT *res,
  ...
  if (CDN_EOK != ret) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
            "Critical error! Wrong value in function parameter\n", 0);
    ret = CDN_EINVAL;
  } else if (res->portOverrideRegs == NULL) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
            "Warning! Port Override register pointer not set.\n", 0);
  } else {
