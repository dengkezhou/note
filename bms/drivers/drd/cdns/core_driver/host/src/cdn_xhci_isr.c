...
static void updateEventPtr(USBSSP_DriverResourcesT *res, uint32_t interrupter) {
  ...
  USBSSP_RingElementT *poolEnd =
      &res->xhciMemRes->eventPool[interrupter][USBSSP_EVENT_QUEUE_SIZE - 1U];

  CPS_CacheFlush((void *)res->eventPtrBuffer[interrupter],
 ...
 */
static void connectStatusChangeConnect(USBSSP_DriverResourcesT *res,
                                       uint32_t portStatus) {
  uint32_t ret;
  ...
  if ((portStatus & USBSSP__PORTSC1USB3__PED_MASK) > 0U) {
    ...
    uint32_t actualSpeed =
        CPS_FLD_READ(USBSSP__PORTSC1USB3, PORTSPEED, portStatus);
...
static void portLinkStateChange(USBSSP_DriverResourcesT *res,
  ...
  if (CPS_FLD_READ(USBSSP__PORTSC1USB3, PLS, portStatus) == 15U /*RESUME*/) {
    ...
    /* PLS=U0 LWS=1 */
    portsc =
        xhciRead32(&res->regs.xhciPortControl[res->actualPort - 1U].portsc);
 ...
 */
static uint32_t portChangeDetect(USBSSP_DriverResourcesT *res,
  ...
  uint32_t portStatus;
  uint8_t portIndex;
  uint32_t maskAllChangeBits =
      USBSSP__PORTSC1USB3__CSC_MASK | USBSSP__PORTSC1USB3__PEC_MASK |
      USBSSP__PORTSC1USB3__WRC_MASK | USBSSP__PORTSC1USB3__PRC_MASK |
      USBSSP__PORTSC1USB3__PLC_MASK | USBSSP__PORTSC1USB3__CEC_MASK |
  ...
  do {
    ...
    if (counter == 0U) {
      ...
      vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT, "%s() timeout after %d\n", __func__,
              USBSSP_CONSECUTIVE_EVENTS);
      break;
    }

    if (res->preportChangeDetectCallback != NULL) {
      vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
              "<%d> Call user defined Port Status Change Event pre callback\n",
              res->instanceNo);
 ...
 */
static void setDequeuePointerEp0(USBSSP_DriverResourcesT *res,
                                 uint64_t newDequeuePtr) {

  vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
          "<%d> SET dequeue pointer of endpoint(%d) to 0x%08x%08x\n",
          res->instanceNo, USBSSP_EP0_CONTEXT_OFFSET,
...
static void commadCompletionEnableSlot(USBSSP_DriverResourcesT *res) {
  ...
  if ((res->actualdeviceSlot >= res->maxDeviceSlot) ||
      (res->actualdeviceSlot >= USBSSP_MAX_DEVICE_SLOT_NUM)) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
            "<%d> actualdeviceSlot (%d) greater than max slots (%d)\n",
            res->instanceNo, res->actualdeviceSlot, res->maxDeviceSlot);
  } else {
    res->xhciMemRes->dcbaa->deviceSlot[res->actualdeviceSlot - 1U] =
 ...
 */
static void commadCompletionResetEpSW(USBSSP_DriverResourcesT *res,
                                      uint8_t epIndex) {
  ...
  if (endpointState == USBSSP_EP_CONTEXT_EP_STATE_STOPPED) {
    ...
    if (epIndex == USBSSP_EP0_CONTEXT_OFFSET) {
      ...
      if (dequeuePtr == NULL) {
        ...
        setDequeuePointerEp0(
            res, le64ToCpu(get64PhyAddrOf32ptr(&dequeuePtr->dword0)));
      }
    } else {
...
static void commadCompletionResetDevice(USBSSP_DriverResourcesT *res) {
  printf("<%d> Reset device completed.\n", res->instanceNo);
  printf("<%d> Device address: %d \n", res->instanceNo, res->devAddress);
 ...
 * @param res driver resources
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Pass parameter res with
 * const specifier, DRV-4333" */
/* parasoft-begin-suppress MISRA2012-RULE-2_7-4 "Parameter res not used in
 * function handleDisableSlot, DRV-4328" */
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Pass parameter res with
 * const specifier, DRV-4334" */
/* parasoft-begin-suppress MISRA2012-RULE-2_7-4 "Parameter res not used in
 * function handleHaltEndpoint, DRV-4329" */
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Pass parameter res with
 * const specifier, DRV-4335" */
/* parasoft-begin-suppress MISRA2012-RULE-2_7-4 "Parameter res not used in
 * function handleHaltStopEndpoint, DRV-4330" */
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Pass parameter res with
 * const specifier, DRV-4336" */
/* parasoft-begin-suppress MISRA2012-RULE-2_7-4 "Parameter res not used in
 * function handleForceEvent, DRV-4331" */
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Pass parameter res with
 * const specifier, DRV-4337" */
/* parasoft-begin-suppress MISRA2012-RULE-2_7-4 "Parameter res not used in
 * function handleDefault, DRV-4332" */
 ...
 */
/* parasoft-begin-suppress MISRA2012-RULE-8_13_a-4 "Pass parameter res with
 * const specifier, DRV-4338" */
static void commadCompletionSuccessfull(USBSSP_DriverResourcesT *res,
                                        uint32_t command) {
  uint32_t commands[] = {
  ...
  uint32_t ch =
      remap(command, commands, (uint32_t)(sizeof(commands) / sizeof(uint32_t)));
 ...
 */
static USBSSP_ProducerQueueT *
getTransferObj(USBSSP_DriverResourcesT *res,
               const USBSSP_RingElementT *eventPtr, uint8_t endpointId) {

  USBSSP_ProducerQueueT *transferObj;

  /* parasoft-begin-suppress MISRA2012-RULE-11_4-4 "uintptr_t converted to
   * USBSSP_RingElementT*, DRV-4286" */
  /* parasoft-begin-suppress MISRA2012-RULE-11_3-2 "uint32_t* converted to
   * uintptr_t*, DRV-4280" */
  uint32_t addrLoCpu = le32ToCpu(eventPtr->dword0);
 ...
 */
static void handleEpCompletion(USBSSP_DriverResourcesT const *res,
                               USBSSP_ProducerQueueT const *transferObj) {
  ...
  if (ringIterator != NULL) {
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
            "<%d> contextIndex:%d numOfBytes: %d, realDataLenSum: %d, "
            "numOfResidue: %d\n",
            res->instanceNo, transferObj->contextIndex, userDataLenSum,
            realDataLenSum, (userDataLenSum - realDataLenSum));
  }
}
...
static uint8_t handleCompleteStatus(USBSSP_DriverResourcesT *res,
  ...
  case USBSSP_TRB_CMPL_MISSED_SRV_ER:
    vDbgMsg(USBSSP_DBG_DRV, DBG_FYI, "<%d> MISSED_SERVICE_ERROR Endpoint(%d)\n",
            res->instanceNo, epIndex);
    /* since we always set IOC for the last TRB of the td, only call completion
     * for the TRB with IOC */
    if (getTrTrbChain(transferObj->dequeuePtr) == 0U) {
      ret = 1U;
    }
    break;
 ...
 */
static uint32_t handleTransferEvent(USBSSP_DriverResourcesT *res,
                                    const USBSSP_RingElementT *eventPtr,
                                    uint8_t endpointId) {
  uint32_t ret = CDN_EOK;

  if ((endpointId < USBSSP_EP0_CONT_OFFSET) ||
      (endpointId >= USBSSP_EP_CONT_MAX)) {
    ret = CDN_EINVAL;
  } else {
     ...
     * object */
    USBSSP_ProducerQueueT *transferObj =
        getTransferObj(res, eventPtr, endpointId);

    vDbgMsg(
        USBSSP_DBG_DRV, DBG_FYI,
        "contextIndex:%d  EvtEpId:%d  CmplCode:%d  Event->xferTrbAddr:0x%16x\n",
        transferObj->contextIndex, getEndpoint(eventPtr),
...
static uint32_t validateDequeuePtr(const USBSSP_ProducerQueueT *queue,
  ...
  uint32_t ret = CDN_EOK;
  USBSSP_RingElementT *poolEnd =
      &queue->ring[(USBSSP_PRODUCER_QUEUE_SIZE - 1U)];
  uintptr_t poolStartAddr =
      (uintptr_t)get64PhyAddrOf32ptr(&queue->ring[0].dword0);
  uintptr_t poolEndAddr = (uintptr_t)get64PhyAddrOf32ptr(&poolEnd->dword0);
  //printf("dequeueAddr = 0x%x, poolStartAddr = 0x%x\n",dequeueAddr, poolStartAddr);
...
static uint32_t checkTranferEventTRB(const USBSSP_DriverResourcesT *res,
  ...
  if (endpointId == USBSSP_EP0_CONTEXT_OFFSET) {
    ...
    uint32_t completionCode =
        getCompletionCode(trb); /* returns code within [0:255] */
 ...
 */
static uint32_t switchFunction(USBSSP_DriverResourcesT *res,
                               USBSSP_RingElementT *eventPtr,
                               uint32_t trbType) {
  uint32_t ret = CDN_EOK;
  /* check event type */
  ...
  case USBSSP_TRB_HOST_CTRL_EVT:
    ...
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT, "<%d> Completion Code: %d\n",
            res->instanceNo,
            le32ToCpu(eventPtr->dword2) >> USBSSP_COMPLETION_CODE_POS);
    break;
 ...
 */
static uint32_t isrCheckInterruptPending(const USBSSP_DriverResourcesT *res,
  ...
  USBSSP_InterrupterT *xhciInterrupter =
      &res->regs.xhciInterrupter[interrupter];
  ...
  vDbgMsg(USBSSP_DBG_DRV, DBG_FYI,
          "<%d> Interrupter(%d) IMAN: %d interruptPending: %d \n",
 ...
 * @return CDN_EOK on success, error otherwise
 */
static uint32_t isrProcessEvent(USBSSP_DriverResourcesT *res,
                                uint32_t interrupter) {
  uint32_t trbType;
 ...
 * @return CDN_EOK on success, error otherwise
 */
static uint32_t isrHandleInterrupter(USBSSP_DriverResourcesT *res,
                                     uint32_t interrupter) {
  ...
  CPS_CacheInvalidate((void *)res->eventPtrBuffer[interrupter],
                      sizeof(USBSSP_RingElementT), 0);

  /* check babble interrupt */
  if (getToogleBit(res->eventPtrBuffer[interrupter]) !=
      res->eventToogleBit[interrupter]) {
    vDbgMsg(USBSSP_DBG_DRV, DBG_CRIT,
            "<%d> Babble interrupt!  interrupter(%d)\n", res->instanceNo,
            interrupter);
    ret = CDN_EPROTO;
  }
  if (ret == CDN_EOK) {
    ...
    } while (getToogleBit(res->eventPtrBuffer[interrupter]) ==
             res->eventToogleBit[interrupter]);
...
uint32_t USBSSP_Isr(USBSSP_DriverResourcesT *res) {
  ...
  if (ret == CDN_EOK) {
    ...
    for (interrupter = 0; interrupter < USBSSP_MAX_NUM_INTERRUPTERS;
         interrupter++) {
      USBSSP_InterrupterT *xhciInterrupter =
          &res->regs.xhciInterrupter[interrupter];
      ...
      if (isrCheckInterruptPending(res, interrupter) != 0U) {
        ...
        vDbgMsg(USBSSP_DBG_DRV, DBG_FYI, "<%d> Processing Interrupter:(%d)\n",
                res->instanceNo, interrupter);
        res->eventPtr =
            res->eventPtrBuffer[interrupter]; /* this is not re-entrant */
