...
/** Position offset for MULT bitfield in endpoint context data structure. The
 * value of MULT is dependent on LEC bit */
...
/** Position offset for CErr bitfield in endpoint context data structure. This
 * bitfield identifies the number of consecutive USB Bus Errors allowed while
 * executing a TD */
...
/** Position offset for max primary stream IDs in endpoint context data
 * structure */
#define USBSSP_EP_CXT_PMAXSTREAMS_POS 10U

/** CErr bitfield in endpoint context data structure is set to '3' in normal
 * operations */
...
/** Position offset for slot state. This field is updated when a device slot
