...
#else
#define APITRACE(format, ...) printf(format, ##__VA_ARGS__); printf("\n");
#define APITRACEPARAM(str, val) \
  { char tmpstr[255]; sprintf(tmpstr, "  %s: %d", str, (int)val); APITRACE(tmpstr); }
#endif
...
H264EncRet H264EncInit(const H264EncConfig * pEncCfg, H264EncInst * instAddr)
{
    ...
    APITRACEPARAM("streamType\n", pEncCfg->streamType);
    APITRACEPARAM("viewMode\n", pEncCfg->viewMode);
    APITRACEPARAM("level\n", pEncCfg->level);
    APITRACEPARAM("refFrameAmount\n", pEncCfg->refFrameAmount);
    APITRACEPARAM("width\n", pEncCfg->width);
    APITRACEPARAM("height\n", pEncCfg->height);
    APITRACEPARAM("frameRateNum\n", pEncCfg->frameRateNum);
    APITRACEPARAM("frameRateDenom\n", pEncCfg->frameRateDenom);
    APITRACEPARAM("scaledWidth\n", pEncCfg->scaledWidth);
    APITRACEPARAM("scaledHeight\n", pEncCfg->scaledHeight);
