...
static void conf(struct menu *menu) {
  ...
  if (prop) {
    ...
    switch (prop->type) {
    case P_MENU:
      if ((input_mode == silentoldconfig || input_mode == listnewconfig ||
...
static void conf_usage(const char *progname) {
  ...
  printf("  --oldconfig             Update a configuration using a provided "
         ".config as base\n");
  ...
  printf("  --savedefconfig <file>  Save the minimal current configuration to "
         "<file>\n");
  printf("  --allnoconfig           New config where all options are answered "
         "with no\n");
  printf("  --allyesconfig          New config where all options are answered "
         "with yes\n");
  printf("  --allmodconfig          New config where all options are answered "
         "with mod\n");
  printf(
  ...
  case randconfig:
    ...
    if ((strcmp(name, "") != 0) && (strcmp(name, "1") != 0)) {
      if (conf_read_simple(name, S_DEF_USER)) {
        fprintf(stderr, _("*** Can't read seed configuration \"%s\"!\n"), name);
        exit(1);
      }
      break;
    }
    ...
    if (conf_read_simple(name, S_DEF_USER) &&
      ...
      fprintf(stderr,
              _("*** KCONFIG_ALLCONFIG set, but no \"%s\" or \"all.config\" "
                "file found\n"),
              name);
      exit(1);
    }
  ...
  case silentoldconfig:
    ...
    do {
      ...
      check_conf(&rootmenu);
             ...
             (input_mode != listnewconfig && input_mode != olddefconfig));
    break;
  }
