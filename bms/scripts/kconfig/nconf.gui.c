...
#define mkattr(name, attr)                                                     \
  do {                                                                         \
    attributes[name] = attr | COLOR_PAIR(name);                                \
  } while (0)
...
int dialog_inputbox(WINDOW *main_window, const char *title, const char *prompt,
  ...
  mvwprintw(form_win, 0, 0, "%s", result + cursor_position - cursor_form_win);
  ...
  while ((res = wgetch(form_win))) {
    ...
    case KEY_BACKSPACE:
      if (cursor_position > 0) {
        memmove(&result[cursor_position - 1], &result[cursor_position],
    ...
    case KEY_DC:
      if (cursor_position >= 0 && cursor_position < len) {
        memmove(&result[cursor_position], &result[cursor_position + 1],
                len - cursor_position + 1);
        len--;
      }
      break;
...
void show_scroll_win(WINDOW *main_window, const char *title, const char *text) {
  ...
  do {
    ...
    if (res == 10 || res == 27 || res == 'q' || res == KEY_F(F_HELP) ||
