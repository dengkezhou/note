...
#define CLINT_BASE          (0xE0000000UL)                            /*!< CLINT Base Address */
...
#define CLINT               ((CLINT_Type   *)     CLINT_BASE  )       /*!< SysTick configuration struct */
...
{
    if (CLINT->MTIMECMP) {
        CLINT->MTIMECMP = CLINT->MTIMECMP + ticks;
    } else {
