...
#define CACHE_MHCR_WBR_Msk                                                     \
  (0x1UL << CACHE_MHCR_WBR_Pos) /*!< CACHE MHCR: WBR Mask */

#define CACHE_MHCR_IBPE_Pos 7U /*!< CACHE MHCR: IBPE Position */
#define CACHE_MHCR_IBPE_Msk                                                    \
  (0x1UL << CACHE_MHCR_IBPE_Pos) /*!< CACHE MHCR: IBPE Mask */

#define CACHE_MHCR_L0BTB_Pos 6U /*!< CACHE MHCR: L0BTB Position */
#define CACHE_MHCR_L0BTB_Msk                                                   \
  (0x1UL << CACHE_MHCR_L0BTB_Pos) /*!< CACHE MHCR: BTB Mask */
...
__STATIC_INLINE void csi_vic_enable_irq(int32_t IRQn) {
  PLIC->PLIC_H0_MIE[IRQn / 32] =
      PLIC->PLIC_H0_MIE[IRQn / 32] | (0x1 << (IRQn % 32));
}
...
__STATIC_INLINE uint32_t csi_coret_config(uint32_t ticks, int32_t IRQn) {
  uint64_t value =
      (((uint64_t)CORET->MTIMECMPH0) << 32) + (uint64_t)CORET->MTIMECMPL0;
...
*/
__STATIC_INLINE void csi_dcache_clean_invalid_range(uint64_t *addr,
                                                    int64_t dsize) {
#if (__DCACHE_PRESENT == 1U)
  int64_t op_size  = dsize + (uint64_t)addr % 64;
  uint64_t op_addr = (uint64_t)addr;
...
/**
  ...
  */
__STATIC_INLINE void csi_cache_set_range(uint64_t index, uint64_t baseAddr,
                                         uint64_t size, uint64_t enable) {
  ;
}
...
/**
  ...
  */
__STATIC_INLINE uint64_t csi_cache_get_access_time(void) { return 0; }
...
/**
  ...
  */
__STATIC_INLINE uint64_t csi_cache_get_miss_time(void) { return 0; }
