...
typedef enum {
  I2S_ERROR_SYNCHRONIZATION =
      (DRV_ERROR_SPECIFIC + 1),  ///< Specified Synchronization not supported
  ...
  I2S_ERROR_FRAME_SYNC_POLARITY, ///< Specified Frame Sync polarity not
                                 ///< supported
...
typedef enum {
  ...
  ///< full duplex mode only for idx0, if used full duplex mode, rx and tx must
  ///< used same rate, mclk_freq, sclk_freq.
...
typedef enum {
  I2S_SCLK_32FS, ///< SCLK Freq = 32 * sample_rate, freq must >= sample_rate *
                 ///< sample_witch
  I2S_SCLK_48FS,
  I2S_SCLK_64FS,
  I2S_SCLK_16FS,
...
typedef struct {
  ...
  uint32_t tx_fifo_empty : 1; ///< Transmit data underflow detected (cleared on
                              ///< start of next send operation)
  uint32_t rx_fifo_full : 1;  ///< Receive data overflow detected (cleared on
                              ///< start of next receive operation)
  uint32_t frame_error : 1; ///< Sync Frame error detected (cleared on start of
                            ///< next send/receive operation)
} i2s_status_t;
...
typedef enum {
  ...
  I2S_EVENT_FRAME_ERROR =
      (0x01 << 6), ///< Sync Frame error in Slave mode (optional)
} i2s_event_e;
...
typedef struct {
  ...
  uint32_t event_frame_error : 1; ///< supports Frame error event:
...
typedef struct {
  ...
  i2s_left_channel_polarity_e
      left_polarity; ///< set ws signel left channle polarity
  i2s_sclk_freq_e
      sclk_freq; ///< I2S sclk freq select. example:32fs = 32 * sample_rate
  i2s_mclk_freq_e
      mclk_freq; ///< I2S mclk freq select. example:256fs = 256 * sample_rate
  int32_t tx_mono_enable; ///< used for tx or full duplex mode, tx mono mode
                          ///< enable. 1 eanble, 0 disable
  int32_t rx_mono_enable; ///< used for rx or full duplex mode,rx mono mode
                          ///< enable. 1 eanble, 0 disable
...
typedef void (*i2s_event_cb_t)(
    int32_t idx, i2s_event_e event,
    void *arg); ///< Pointer to \ref i2s_event_cb_t : i2s Event call back.

/**
  \brief       Initialize I2S Interface. 1. Initializes the resources needed for
  the I2S index 2.registers event callback function \param[in]   idx  i2s index
...
/**
  \brief       De-initialize I2S Interface. stops operation and releases the
  software resources used by the interface \param[in]   handle i2s handle to
...
*/
uint32_t csi_i2s_send(i2s_handle_t handle, const uint8_t *data,
                      uint32_t length);

/**
  \brief       receiving data from i2s receiver.
  \param[in]   handle       i2s handle to operate.
  \param[in]   data         Pointer to buffer for data to receive from i2s
  receiver \param[in]   data_size    size of receiver data \return      receive
  data size.(bytes)
*/
