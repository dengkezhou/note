...
*/
typedef struct {
  uint32_t unalign_addr : 1; ///< support for unalign address transfer when
                             ///< memory is source
} dma_capabilities_t;

typedef enum {
  DMA_STATE_FREE = 0, ///< DMA channel not yet initialized or disabled
  DMA_STATE_READY, ///< DMA channel process success and ready for use, but not
                   ///< start yet
...
typedef enum {
  ...
  DMA_EVENT_TRANSFER_MODE_DONE =
      2, ///< transfer complete in a certain dma trigger mode.
  DMA_EVENT_CAHNNEL_PEND =
      3, ///< it happens when there is a low priority channel was preempted by a
...
typedef enum { DMA_DIR_DEST = 0, DMA_DIR_SOURCE } dma_single_dir_e;

typedef enum { DMA_ADDR_LITTLE = 0, DMA_ADDR_BIG } dma_addr_endian_e;
...
typedef struct {
  ...
  dma_addr_endian_e
      src_endian; ///< source read data little-big endian change control.
  dma_addr_endian_e
      dst_endian; ///< destination write data little-big endian change control.
  ...
  uint8_t preemption; ///< determine whether if a channel can be preempted by a
                      ///< higher priority channel, 0 -- not allow preempt, 1 --
                      ///< allow preempt.
  dma_trans_type_e type;      ///< transfer type
  dma_trig_trans_mode_e mode; ///< channel trigger mode
  dma_channel_req_mode_e
      ch_mode; ///< software or hardware to tigger dma channel work.
  dma_single_dir_e single_dir; ///< after select single mode control for
                               ///< source(read) or destination(write) transfer.
  uint32_t group_len;      ///< group transaction length (unit: bytes) when use
                           ///< DMA_GROUP_TRIGGER mode.
...
typedef void (*dma_event_cb_t)(
    int32_t ch, dma_event_e event,
    void *arg); ///< Pointer to \ref dma_event_cb_t : dmac event call back.
...
/**
  ...
  \return       error code
 */
int32_t csi_dma_config_channel(int32_t ch, dma_config_t *config,
                               dma_event_cb_t cb_event, void *cb_arg);
...
/**
  ...
  \return       error code
*/
int32_t csi_dma_cyclic_prep(int32_t ch, void *psrcaddr, void *pdstaddr,
                            uint32_t cycle_len, uint32_t cycle_num);
/**
  \brief       start generate dma channel cyclic transfer.
  \param[in]   ch   dma channel num
*/
