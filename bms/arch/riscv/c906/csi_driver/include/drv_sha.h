...
typedef void (*sha_event_cb_t)(
    int32_t idx,
    sha_event_e
        event); ///< Pointer to \ref sha_event_cb_t : SHA Event call back.
...
/**
  \brief       Initialize SHA Interface. 1. Initializes the resources needed for
  the SHA interface 2.registers event callback function \param[in]   idx index
  of sha \param[in]   context Pointer to the buffer storing the sha context
  \param[in]   cb_event  event callback function \ref sha_event_cb_t
  \return      return sha handle if success
*/
sha_handle_t csi_sha_initialize(int32_t idx, void *context,
                                sha_event_cb_t cb_event);

/**
  \brief       De-initialize SHA Interface. stops operation and releases the
  software resources used by the interface \param[in]   handle  sha handle to
...
/**
  ...
  \return      error code
*/
int32_t csi_sha_config(sha_handle_t handle, sha_mode_e mode,
...
/**
  ...
  \return      error code
*/
int32_t csi_sha_update(sha_handle_t handle, void *context, const void *input,
                       uint32_t len);
