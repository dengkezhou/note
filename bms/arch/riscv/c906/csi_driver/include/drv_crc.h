...
#define CRC_ERROR_MODE                                                         \
  (DRV_ERROR_SPECIFIC + 1) ///< Specified Mode not supported
...
typedef void (*crc_event_cb_t)(
    int32_t idx,
    crc_event_e
        event); ///< Pointer to \ref crc_event_cb_t : CRC Event call back.
...
/**
  \brief       Initialize CRC Interface. 1. Initializes the resources needed for
  the CRC interface 2.registers event callback function \param[in]   idx  device
  id \param[in]   cb_event event callback function \ref crc_event_cb_t \return
  return crc handle if success
*/
crc_handle_t csi_crc_initialize(int32_t idx, crc_event_cb_t cb_event);

/**
  \brief       De-initialize CRC Interface. stops operation and releases the
  software resources used by the interface \param[in]   handle  crc handle to
...
/**
  ...
  \return      error code
*/
int32_t csi_crc_config(crc_handle_t handle, crc_mode_e mode,
                       crc_standard_crc_e standard);

/**
  \brief       calculate crc. this function will pad zero if input data is not
  word aligned \param[in]   handle  crc handle to operate. \param[in]   in
  Pointer to the input data \param[out]  out     Pointer to the result.
  \param[in]   len     intput data len.
  \return      error code
*/
int32_t csi_crc_calculate(crc_handle_t handle, const void *in, void *out,
                          uint32_t len);
