...
/****** TRNG Event *****/
typedef enum {
  TRNG_EVENT_DATA_GENERATE_COMPLETE =
      0 ///< True random number generates completely
} trng_event_e;
typedef void (*trng_event_cb_t)(
    int32_t idx,
    trng_event_e
        event); ///< Pointer to \ref trng_event_cb_t : TRNG Event call back.
...
/**
  ...
  device id \param[in]   cb_event  event call back function \ref trng_event_cb_t
