...
#define TIMER_NULL_PARAM_CHK(para)                                             \
  HANDLE_PARAM_CHK(para, ERR_TIMER(DRV_ERROR_PARAMETER))
...
extern int32_t target_get_timer(int32_t idx, uint64_t *base, uint32_t *irq,
                                void **handler);
...
int32_t csi_timer_start(timer_handle_t handle) {
  ...
  if (((timer_priv->timeout * drv_get_timer_freq(timer_priv->idx)) / 1000000) >
      0xffffffff) {
    return ERR_TIMER(DRV_ERROR_PARAMETER);
  }
  ...
  if (min_us) {
    ...
    load = (uint32_t)(((uint64_t)(timer_priv->timeout) *
                       drv_get_timer_freq(timer_priv->idx)) /
                      1000000);
  }
  ...
  addr->TxControl |=
      DW_TIMER_TXCONTROL_ENABLE; /* enable the corresponding timer */
...
int32_t csi_timer_resume(timer_handle_t handle) {
  ...
  addr->TxControl &=
      ~DW_TIMER_TXCONTROL_ENABLE; /* stop the corresponding timer */
  addr->TxControl &=
      DW_TIMER_TXCONTROL_ENABLE; /* restart the corresponding timer */
