...
typedef struct {
  ...
  uint32_t error : 1; ///< Read/Program/Erase error flag (cleared on start of
                      ///< next operation)
} spiflash_status_t;
...
typedef void (*spiflash_event_cb_t)(
    int32_t idx,
    spiflash_event_e event); ///< Pointer to \ref spiflash_event_cb_t : SPIFLASH
                             ///< Event call back.
...
/**
  \brief       Initialize SPIFLASH Interface. 1. Initializes the resources
  needed for the SPIFLASH interface 2.registers event callback function
  \param[in]   idx  device id
  \param[in]   cb_event  Pointer to \ref spiflash_event_cb_t
  \return      pointer to spiflash handle
*/
spiflash_handle_t csi_spiflash_initialize(int32_t idx,
                                          spiflash_event_cb_t cb_event);

/**
  \brief       De-initialize SPIFLASH Interface. stops operation and releases
  the software resources used by the interface \param[in]   handle  spiflash
  handle to operate. \return      error code
*/
...
/**
  ...
  \return      error code
*/
int32_t csi_spiflash_config_data_line(spiflash_handle_t handle,
                                      spiflash_data_line_e line);
...
*/
int32_t csi_spiflash_read(spiflash_handle_t handle, uint32_t addr, void *data,
                          uint32_t cnt);
...
/**
  ...
  \param[in]   handle  spiflash handle to operate.
  \param[in]   addr  Data address.
  \param[in]   data  Pointer to a buffer containing the data to be programmed to
  Flash.. \param[in]   cnt   Number of data items to program. \return number of
  data items programmed or error code
*/
int32_t csi_spiflash_program(spiflash_handle_t handle, uint32_t addr,
