...
extern int32_t drv_pin_config_mode(port_name_e port, uint8_t offset,
                                   gpio_mode_e pin_mode);

#define ERR_GPIO(errno) (CSI_DRV_ERRNO_GPIO_BASE | errno)
#define GPIO_NULL_PARAM_CHK(para)                                              \
  HANDLE_PARAM_CHK(para, ERR_GPIO(DRV_ERROR_PARAMETER))
...
extern int32_t target_gpio_port_init(port_name_e port, uint32_t *base,
                                     uint32_t *irq, void **handler,
                                     uint32_t *pin_num);
...
static int32_t gpio_read(void *port, uint32_t *value) {
  dw_gpio_priv_t *gpio_priv = port;
  dw_gpio_control_reg_t *gpio_control_reg =
      (dw_gpio_control_reg_t *)(gpio_priv->base + 0x30);
 ...
 */
static int32_t gpio_set_irq_mode(gpio_pin_handle_t pin,
  ...
  dw_gpio_control_reg_t *gpio_control_reg =
      (dw_gpio_control_reg_t *)(port_handle->base + 0x30);
...
void dw_gpio_irqhandler(int idx) {
  ...
  dw_gpio_control_reg_t *gpio_control_reg =
      (dw_gpio_control_reg_t *)(gpio_handle[idx].base + 0x30);
...
#endif
      dw_gpio_pin_priv_t *gpio_pin_priv =
          (dw_gpio_pin_priv_t *)&gpio_pin_handle[pin_idx];
...
/**
  \brief       Initialize GPIO module. 1. Initializes the resources needed for
  the GPIO handle 2.registers event callback function 3.get gpio_port_handle
...
/**
  \brief       De-initialize GPIO handle. stops operation and releases the
  software resources used by the handle \param[in]   handle   gpio port handle
  to operate. \return      error code
*/
...
*/
gpio_pin_handle_t csi_gpio_pin_initialize(int32_t gpio_pin,
...
/**
  \brief       De-initialize GPIO pin handle. stops operation and releases the
  software resources used by the handle \param[in]   handle   gpio pin handle to
...
/**
  ...
  \return      error code
*/
int32_t csi_gpio_power_control(gpio_pin_handle_t handle,
                               csi_power_stat_e state) {
  GPIO_NULL_PARAM_CHK(handle);

#ifdef CONFIG_LPM
  dw_gpio_pin_priv_t *gpio_pin_priv = (dw_gpio_pin_priv_t *)handle;
  power_cb_t callback               = {.wakeup       = do_wakeup_sleep_action,
                         .sleep        = do_prepare_sleep_action,
                         .manage_clock = manage_clock};
  return drv_soc_power_control(&gpio_handle[gpio_pin_priv->portidx], state,
                               &callback);
#else
  return ERR_GPIO(DRV_ERROR_UNSUPPORTED);
#endif
...
/**
  ...
  \return      error code
*/
int32_t csi_gpio_pin_config(gpio_pin_handle_t handle, gpio_mode_e mode,
                            gpio_direction_e dir) {
  GPIO_NULL_PARAM_CHK(handle);
...
/**
  ...
  \return      error code
*/
int32_t csi_gpio_pin_set_irq(gpio_pin_handle_t handle, gpio_irq_mode_e mode,
                             bool enable) {
  GPIO_NULL_PARAM_CHK(handle);
