...
int serial_pm_suspend(void *privdata) { return 0; }

int serial_pm_resume(void *privdata) {
  console_init(0);
  return 0;
}

SIMPLE_PM_OPS(serial, serial_pm_suspend, serial_pm_resume, NULL);

int init_serial(void) {
  console_init(0);
  // register pm interface
  itcs_register_pm(&pm_serial);
  return 0;
}
