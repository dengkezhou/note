#ifndef __CSI_CONFIG_H__
#define __CSI_CONFIG_H__
#define CONFIG_ARCH_RV32               1
#define CONFIG_RV32_CORETIM            1
#define CONFIG_SYSTEM_SECURE           1
#define CONFIG_CHIP_SMARTH_RV32        1
#define CONFIG_BOARD_SMARTH_C920_EVB   1
#define CONFIG_BOARD_NAME_STR          "smarth_c920_evb"
#define CONFIG_KERNEL_FREERTOS         1
#define CONFIG_HAVE_VIC                1
#define CONFIG_ARCH_INTERRUPTSTACK     8192
#define CONFIG_NEWLIB_WRAP             1
#define CONFIG_USER_DEFINED_LD_DIR_STR ""

#define CONFIG_CPU_C920V    1
#define CONFIG_RISCV_VECTOR_LEN   128
#endif
