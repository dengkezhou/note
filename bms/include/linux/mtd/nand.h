...
#define NAND_SUBPAGE_READ(chip)                                                \
  ((chip->ecc.mode == NAND_ECC_SOFT) && (chip->page_shift > 9))
 ...
 * progress used instead of the per chip wait queue when a hw controller is
 * available
 */
 ...
 * @erase_cmd:		[INTERN] erase command write function, selectable due to
 *AND support
 * @scan_bbt:		[REPLACEABLE] function to scan bad block table
 * @chip_delay:		[BOARDSPECIFIC] chip dependent delay for transfering
 *data from array to read regs (tR)
 * @wq:			[INTERN] wait queue to sleep on if a NAND operation is
 *in progress
 ...
 * @options:		[BOARDSPECIFIC] various chip options. They can partly be
...
extern const struct nand_id_link nand_id_links[];
extern const struct nand_flash_dev prv_nand_flash_ids[];
...
static inline struct platform_nand_chip *
get_platform_nandchip(struct mtd_info *mtd) {
