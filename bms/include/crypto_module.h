...
#define BLKC_CHAIN_MODE                                                        \
  1 /* block cipher chain mode  [5:1]  1:ECB 2:CBC 3:CFB 4:OFB 5:CTR */
...
#define RC4_SW_KSA_DONE                                                        \
  29 /* aftern sw set the sw , it should be written to 1 */
#define RC4_RST                                                                \
  31 /* when you set sw and re-init, you need to reset the rc4                 \
      */
...
extern uint32_t block_cipher_aes_init(uint32_t crypto, uint32_t keymode,
                                      uint8_t *key, uint8_t *iv);
extern uint32_t block_cipher_operation(uint8_t *indata, uint8_t *outdata,
                                       uint32_t len);
