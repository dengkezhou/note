...
ee_u16 core_bench_list(core_results *res, ee_s16 finder_idx) {
  ...
  info.idx = finder_idx;
  /* find <find_num> values in the list, and change the list each time (reverse
   * and cache if value found) */
...
list_head *core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed) {
  ...
  for (i = 0; i < size; i++) {
    ...
    info.data16 = (dat << 8) | dat; /* fill the data with actual data and upper
                                       bits with rebuild value */
    core_list_insert_new(list, &info, &memblock, &datablock, memblock_end,
                         datablock_end);
  }
