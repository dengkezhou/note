...
#error "Please define a way to initialize a memory block."
...
#if HAS_FLOAT
  ee_printf("Total time (secs): %lld\n", (long)time_in_secs(total_time));
  if (time_in_secs(total_time) > 0)
    ee_printf("Iterations/Sec   : %lld\n",
              (long)(default_num_contexts * results[0].iterations /
  ...
  if (total_errors == 0) {
    ee_printf("Correct operation validated. See README.md for run and "
              "reporting rules.\n");
#if HAS_FLOAT
    if (known_id == 3) {
      ee_printf("CoreMark 1.0 : %lld / %s %s",
                (long)(default_num_contexts * results[0].iterations /
                       time_in_secs(total_time)),
