...
#define GETMYTIME(_t)        *_t = get_timer(0)
...
void portable_init(core_portable *p, int *argc, char *argv[]) {
  ...
  if ((nargs > 1) && (*argv[1] == 'M')) {
    ...
    /* Shift args since first arg is directed to the portable part and not to
     * coremark main */
    --nargs;
    for (i = 1; i < nargs; i++)
      argv[i] = argv[i + 1];
...
ee_u8 core_start_parallel(core_results *res) {
  ...
  if (res->port.pid == 0) { /* benchmark child */
    ...
    if (-1 == res->port.sock) /* if socket failed to initialize, exit */ {
      ...
      int bytes_sent = sendto(res->port.sock, &(res->crc), buffer_length, 0,
